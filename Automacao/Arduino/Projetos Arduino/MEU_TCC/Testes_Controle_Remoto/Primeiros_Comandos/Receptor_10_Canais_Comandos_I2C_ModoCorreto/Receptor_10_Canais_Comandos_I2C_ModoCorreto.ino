  ...
  if ( radio.available() ) // Se o Radio estiver disponivel, ou seja, recebendo informação do transmissor imprime as informações
  {
    ...
    while (!done)
    {
      ...
      if (SINAIS[3] != 0)
      {
        ...
        acelerador = map(SINAIS[3], 10, 1023, 1000, 2000);
        if (acelerador > 2000) {
          acelerador = 1000;
        }
        if (acelerador < 1000) {
       ...
       // Mapeando aileron
        if (SINAIS[0]!=0 && modo!=0)
        {
         v_aileron = map(SINAIS[0], 14, 1020, 1000, 2000);
         //Serial.println(SINAIS[0]); 

         if (v_aileron>2000){v_aileron = 2000;}
         if (v_aileron<1000){v_aileron = 1000;}
         
         v_aileron = v_aileron/10;
         
         //Serial.println(v_aileron); 
         
         if (v_aileron>200){v_aileron = 200;}
         if (v_aileron<100){v_aileron = 100;}
         ...
         if (v_aileron>150) // Direita
         {
          aileron = map(v_aileron,150,200,100,500);
          
          vel_motor1 = (vel_motor1);
          vel_motor2 = (vel_motor2+aileron);
          vel_motor3 = (vel_motor3+aileron);
          vel_motor4 = (vel_motor4);
         }
         if (v_aileron<150) // Esquerda
         {
          aileron = map(v_aileron,150,100,100,500);
          
          vel_motor1 = (vel_motor1+aileron);
          vel_motor2 = (vel_motor2);
          vel_motor3 = (vel_motor3);
          vel_motor4 = (vel_motor4+aileron);
         }
        }
        ...
        // Mapeando profundor
        if (SINAIS[1]!=0 && modo!=0)
        {
        v_profundor = map(SINAIS[1], 10, 1023, 1000, 2000);
        //Serial.println(SINAIS[1]); 

        if (v_profundor>2000){v_profundor = 2000;}
        if (v_profundor<1000){v_profundor = 1000;}

        v_profundor = v_profundor/10;
        
        if (v_profundor>200){v_profundor = 200;}
        if (v_profundor<100){v_profundor = 100;}
 
        //Serial.println(v_profundor); 
        ...
        if (v_profundor>150) // descer frente
        {
          profundor = map(v_profundor,150,200,100,500);
         
         vel_motor1 = (vel_motor1);
         vel_motor2 = (vel_motor2);
         vel_motor3 = (vel_motor3+profundor);
         vel_motor4 = (vel_motor4+profundor);
        }

        if (v_profundor<150) // subir para tras
        {
          profundor = map(v_profundor,150,100,100,500);
          
         vel_motor1 = (vel_motor1+profundor);
         vel_motor2 = (vel_motor2+profundor);
         vel_motor3 = (vel_motor3);
         vel_motor4 = (vel_motor4);
        }
    }
        ...
        // Mapeando leme
        if (SINAIS[2]!=0 && modo!=0)
        {
        v_leme = map(SINAIS[2], 10, 1023, 1000, 2000);
        //Serial.println(SINAIS[2]); 
        
        if (v_leme>2000){v_leme = 2000;}
        if (v_leme<1000){v_leme = 1000;}

        v_leme = v_leme/10;

        if (v_leme>200){v_leme = 200;}
        if (v_leme<100){v_leme = 100;}

        if (v_leme==150)
        {
          leme = 0;
        }
        if (v_leme>150) // gira para direita
        {
         
         leme = map(v_leme,150,200,100,500);
         
        vel_motor1 = (vel_motor1+leme);
        vel_motor2 = (vel_motor2-leme);
        vel_motor3 = (vel_motor3+leme);
        vel_motor4 = (vel_motor4-leme);
        }

        if (v_leme<150) // gira para esquerda
        {
         leme = map(v_leme,150,100,100,500);
      ...
      if ( (v_profundor > 140 && v_profundor < 160 )&& ( v_aileron >140 && v_aileron <160 ) && (v_leme>140 && v_leme < 160 ) && estabilidade == 1 )//& Ultima_distanciaM != 0.0 )
      {
        ...
        Acelerometro_Eixo_X = map(Acelerometro_Eixo_X, -16300, 16300, -10, 10);
        Acelerometro_Eixo_Y = map(Acelerometro_Eixo_Y, -16300, 16300, -10, 10);
        Acelerometro_Eixo_Z = map(Acelerometro_Eixo_Z, -16300, 16300, -10, 10);

        
        
        // Drone caindo para frente
        if (  Acelerometro_Eixo_X < 0 )
        {
          vel_motor1 = (vel_motor1 - ( Acelerometro_Eixo_X*10));
          vel_motor2 = (vel_motor2 - ( Acelerometro_Eixo_X*10));
          vel_motor3 = (vel_motor3 + ( Acelerometro_Eixo_X*10)); //  ????????
          vel_motor4 = (vel_motor4 + ( Acelerometro_Eixo_X*10)); //  ????????
        }
        ...
        if (  Acelerometro_Eixo_Y < 0 )
        {
          vel_motor1 = (vel_motor1 + (Acelerometro_Eixo_Y*10)); //  ????????
          vel_motor2 = (vel_motor2 - (Acelerometro_Eixo_Y*10));
          vel_motor3 = (vel_motor3 - (Acelerometro_Eixo_Y*10));
          vel_motor4 = (vel_motor4 + (Acelerometro_Eixo_Y*10)); //  ????????
        }
        ...
        if ( vel_motor2 > 179) {vel_motor2 = 179;}
        if ( vel_motor3 > 179) {vel_motor3 = 179;}
        if ( vel_motor4 > 179) {vel_motor4 = 179;}
    ...
    /*
        ...
        Serial.print(",");
        Serial.print(vel_motor4);
        Serial.print(",");
        Serial.print(Acelerometro_Eixo_X);
        Serial.print(",");
        Serial.print(Acelerometro_Eixo_Y);
        Serial.print(",");
        Serial.println(Acelerometro_Eixo_Z);
   */
      ...
      else
      {
       ...
       //Serial.print(v_leme);Serial.print(" , ");Serial.println(leme);
       //Serial.print(v_profundor);Serial.print(" , ");Serial.println(profundor);
       //Serial.print(v_aileron);Serial.print(" , ");Serial.println(aileron);
...
/*
     ...
     */ 
      ...
      if (SINAIS[5] == 1000 && vezes_estabilidade < 20 && estabilidade == 0 ) {
        vezes_estabilidade++;
      }
      if (SINAIS[5] == 500 && estabilidade == 1) {
        vezes_estabilidade++;
      }
      if (SINAIS[5] == 1000 && estabilidade == 1) {
        vezes_estabilidade = 1;
      }
      ...
      if (SINAIS[7] == 1000 && vezes_modo < 20 && modo == 0 ) {
        vezes_modo++;
      }
      if (SINAIS[7] == 500 && modo == 1) {
        vezes_modo++;
      }
      if (SINAIS[7] == 1000 && modo == 1) {
        vezes_modo = 1;
      }
      ...
      //  MODO ILUMINAÇÃO
      if (SINAIS[8] == 1000 && vezes_luz < 20 && luz == 0 ) {
        vezes_luz++;
      }
      if (SINAIS[8] == 500 && luz == 1) {
        vezes_luz++;
      }
      if (SINAIS[8] == 1000 && luz == 1) {
        vezes_luz = 1;
      }
      ...
      if (SINAIS[9] == 1000 && vezes_modo < 20 && modo == 0 ) {
        vezes_modo++;
      }
      if (SINAIS[9] == 500 && modo == 1) {
        vezes_modo++;
      }
      if (SINAIS[9] == 1000 && modo == 1) {
        vezes_modo = 1;
      }
