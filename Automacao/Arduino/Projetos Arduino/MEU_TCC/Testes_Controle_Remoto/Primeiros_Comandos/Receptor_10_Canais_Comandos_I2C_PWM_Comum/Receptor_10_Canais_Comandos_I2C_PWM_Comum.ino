...
void loop()
{
  if ( radio.available() ) // Se o Radio estiver disponivel, ou seja, recebendo informação do transmissor imprime as informações
  {
    ...
    while (!done)
    {
      ...
      if (SINAIS[3] != 0)
      {
        acelerador = map(SINAIS[3], 10, 1023, 20, 179);
        if (acelerador > 179) {
          acelerador = 20;
        }
        if (acelerador < 20) {
        ...
        if (SINAIS[0]!=0)
        {
         v_aileron = map(SINAIS[0], 15, 1000, 0, 41);
         if (v_aileron>41){v_aileron = 21;}
         if (v_aileron<0){v_aileron = 0;}
         ...
         if (v_aileron>21)
         {
          aileron = map(v_aileron,22,37,1,10);
         ...
         if (v_aileron<21)
         {
          aileron = map(v_aileron,20,3,1,10);
        ...
        if (SINAIS[1]!=0)
        {
        v_profundor = map(SINAIS[1], 1, 950, 0, 41);
        if (v_profundor>41){v_profundor = 21;}
        if (v_profundor<0){v_profundor = 0;}
        ...
        if (v_profundor>21) // descer frente
        {
          profundor = map(v_profundor,22,40,1,10);
        ...
        if (v_profundor<21) // subir para tras
        {
          profundor = map(v_profundor,20,4,1,10);
        ...
        if (SINAIS[2]!=0)
        {
        v_leme = map(SINAIS[2], 10, 1000, 0, 41);
        if (v_leme>41){v_leme = 21;}
        if (v_leme<0){v_leme = 0;}

        if (v_leme==21)
        {
          leme = 0;
        }
        if (v_leme>21) // gira para direita
        {
         
         leme = map(v_leme,22,38,1,10);
        ...
        if (v_leme<21) // gira para esquerda
        {
         leme = map(v_leme,20,3,1,10);
      ...
      if ( (v_profundor > 19 && v_profundor < 23 )&& ( v_aileron >19 && v_aileron <23 ) && (v_leme>19 && v_leme < 23 ) && estabilidade == 1 )//& Ultima_distanciaM != 0.0 )
      {
