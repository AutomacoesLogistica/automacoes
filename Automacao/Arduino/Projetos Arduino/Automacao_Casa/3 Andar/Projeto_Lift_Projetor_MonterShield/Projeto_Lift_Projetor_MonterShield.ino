...
long intervalo_subir = 999999;     // Tempo em ms do intervalo a ser executado
long intervalo_descer = 999999;     // Tempo em ms do intervalo a ser executado
...
#define BRAKEGND 3
#define CS_THRESHOLD 20   // Definição da corrente de segurança
...
#define tRampa_TelaFrete 0 // Define o tempo de rampa de translação da ponte tanto para frente quanto para traz
...
void parar_subida()
{
 ...
 { 
  ...
  // Envia a mensagem > projetor_aberto <  no RS485
  digitalWrite(transmitir, HIGH);    // Habilita a transmissão RS485
  RS485.print("projetor_aberto");
...
void parar_descida()
{
 ...
 { 
  ...
  // Envia a mensagem > projetor_fechado <  no RS485
  digitalWrite(transmitir, HIGH);    // Habilita a transmissão RS485
  RS485.print("projetor_fechado");
  digitalWrite(transmitir, LOW);    // Desabilita a transmissão RS485
  Parar_Descer();
 }
...
void destravar()
{
 ...
 motorGo(1, CCW, 255);// Aumento do o PWM do motor até 255 
 delay(500);
 motorGo(1, CCW, 0);// Para Motor
 digitalWrite(inApin, LOW);
 digitalWrite(inBpin, LOW);
 analogWrite(6, 0);
 bb = 0;
...
void loop() 
{
  ...
  if((v_descer == true && fc_Descida == false) || (Tela_F_Subir == 0 && Tela_F_Descer == 1))
  {
   ...
   { 
    ...
    valor_descer = 90; // Reduz a velocidade da descida para chegar e atuar devagar o fim de curso
  ...
  if((v_subir == true && fc_Subida == false)||(Tela_F_Subir == 1 && Tela_F_Descer == 0))
  {
   ...
   { 
    UltimoMillis = AtualMillis;    // Salva o tempo atual
    valor_subir = 254; // Reduz a velocidade da subida e atuar devagar o fim de curso
  ...
  readString.trim();
  if (readString.length() >0 && (readString.indexOf("subir") >=0 || readString.indexOf("parar") >=0 || readString.indexOf("descer") >=0  || readString.indexOf("destravar") >=0))
  {
   ...
   {
    if(digitalRead(2)==HIGH){destravar();}//Destrava o lift para descer
   ...
   if (readString.indexOf("destravar") >=0)
   {
    manual = false;
    destravar();
   }
...
if(analogRead(A0)<300 && Tela_F_Subir == 0 && fc_Descida == false) // descer a tela manualmente
{
 manual = true;
...
if(analogRead(A0)>1000 && Tela_F_Descer == 0 && fc_Subida == false) // subindo a tela manualmente
{
 manual = true; 
...
// ATUA OS COMANDOS RESPECTIVOS    ATUA OS COMANDOS RESPECTIVOS   ATUA OS COMANDOS RESPECTIVOS   ATUA OS COMANDOS RESPECTIVOS
 ...
 if ((Tela_F_Subir == 1 || v_subir==true && fc_Subida == false)&& digitalRead(2)==LOW)
 {
  ...
  if(analogRead(cspin)>162)// Protecao para se agarrar parar
  {
   Serial.println("Desarmou");
   digitalWrite(inApin, LOW);
   digitalWrite(inBpin, LOW);
  ...
  // MOTOR 0 E EM CW 
  for(int e = 254;e<255;e++)
  {
   if (analogRead(A0)>1000 || v_subir==true && fc_Subida == false)
   {
    if ( aa == 0 )
    {
     motorGo(1, CW, e);// Aumento do o PWM do motor até 255
     em_rampa_subindo = true; 
     delay(3);//Tempo de Rampa
 ...
 if (( Tela_F_Descer == 1 || v_descer == true && fc_Descida == false)&& digitalRead(3)==LOW)
 {
  ...
  for(int f = 50;f<90;f++)
  {     
   if (analogRead(A0)<300 || v_descer == true)
   {
    if ( bb == 0 )
    {
     ...
     if ( f == 89){bb = 1;em_rampa_descendo = false;}
     f++;
     valor_subir = f; 
    }
   }
