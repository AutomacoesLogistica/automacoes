/*
   ...
   MODULO DO QUARTO CASAL IP 192.168.2.208
...
byte ip[] = { 192, 168, 2, 208 }; // Define o IP do arduino CADA UM DEVE TER O SEU
String ValorIP = "192.168.2.208"; // Colocar o mesmo que ip, este serve para impressao via mqtt
String id = "quartocasal"; // SEMPRE em minusculo
...
#define mqtt_topico_sub           "dev/test/minhacasa/quartocasal"
#define TOPICO_PUBLISH   "dev/test/minhacasa"


EthernetClient ArduinoMQTT_quartocasal; // Nome de cada Servidor UNICO para cada Arduino
PubSubClient client(ArduinoMQTT_quartocasal); // Nome de cada Servidor UNICO para cada Arduino
...
void reconectar() {
  ...
  while (!client.connected())
  {
    conectado = strlen(servidor_mqtt_usuario) > 0 ?
                client.connect("ArduinoMQTT_quartocasal", servidor_mqtt_usuario, servidor_mqtt_senha) :
                client.connect("ArduinoMQTT_quartocasal");
    if (conectado)
    {
      digitalWrite(2, 1);
