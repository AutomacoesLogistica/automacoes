/*
   ...
   MODULO DO QUARTO 01 IP 192.168.2.203
...
byte ip[] = { 192, 168, 2, 203 }; // Define o IP do arduino CADA UM DEVE TER O SEU
String ValorIP = "192.168.2.203"; // Colocar o mesmo que ip, este serve para impressao via mqtt
String id = "quarto1"; // SEMPRE em minusculo
...
#define mqtt_topico_sub           "dev/test/minhacasa/quarto1"
#define TOPICO_PUBLISH   "dev/test/minhacasa"


EthernetClient ArduinoMQTT_quarto1; // Nome de cada Servidor UNICO para cada Arduino
PubSubClient client(ArduinoMQTT_quarto1); // Nome de cada Servidor UNICO para cada Arduino
...
void reconectar() {
  ...
  while (!client.connected())
  {
    conectado = strlen(servidor_mqtt_usuario) > 0 ?
                client.connect("ArduinoMQTT_quarto1", servidor_mqtt_usuario, servidor_mqtt_senha) :
                client.connect("ArduinoMQTT_quarto1");
    if (conectado)
    {
      digitalWrite(2, 1);
