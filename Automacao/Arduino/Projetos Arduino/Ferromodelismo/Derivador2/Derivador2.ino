...
int pos_reto = 120;//Variável que armazena a posição do servo
int pos_derivado = 78;//Variável que armazena a posição do servo
int x = 120;
String posicao = "Reto";
...
void loop() 
{
 ...
 if ( radio.available() ) // Se o Radio estiver disponivel, ou seja, recebendo informação do transmissor imprime as informações
 {
  ...
  while (!done)
  {
   ...
   int v_posicao = (SINAIS[9]); // status desse derivador
