...
void loop () 
{
 ...
 int vvalor = map(valor_saida_PWM, 0, 255, 0, 120);
 int velocidade = map(valor_saida_PWM, 0, 255, 0, 100);
...
valor = "";
 ...
 display.setFont(ArialMT_Plain_10);
 String msg = "Velocidade = " + String(velocidade) + " %" ;
 display.drawString(0 ,0 ,msg);   
 display.setFont(ArialMT_Plain_24);
 for(int x=0;x<=vvalor;x++)
 {
  display.drawString(x ,10 ,"|");  
 }

display.setFont(ArialMT_Plain_10);
 display.drawString(10 ,40 ,"L1 = OFF");  
display.drawString(65 ,40 ,"L2 = OFF");  
 display.drawString(0 ,50 ,"Frente");  
 display.display();
