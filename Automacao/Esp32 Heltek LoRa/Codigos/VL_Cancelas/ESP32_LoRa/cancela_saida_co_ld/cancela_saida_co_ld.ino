...
void loraData()
{
 ...
 if ( packet == "05_pulso" && manter == false)
 {
 ...
 if ( packet == "05_manter" && manter == false)
 {
  digitalWrite(saida_pulso,HIGH);
  manter = true;
 }
 if ( packet == "05_normalizar" && manter == true)
 {
  digitalWrite(saida_pulso,LOW);
  manter = false;
 } 
...
// MONITORA O SINAL DE ABERTURA VINDAS PELO TOTEM **********************************************************************************************************
 ...
 if(digitalRead(entrada_pulso_totem)==LOW)
 {
  ...
  {
   LoRa.beginPacket();
   LoRa.print("05_totem");   // Envia que a cancela abriu via totem, para finalizar os dados no banco
   LoRa.endPacket();
   delay(30);
  }
  delay(2000);
...
// MONTORA VANDALISMO *************************************************************************************************************************************
 if(digitalRead(entrada_sinal_vandalismo)==LOW)
  ...
  while(digitalRead(entrada_sinal_vandalismo)==LOW)
  {
   ...
   {
    LoRa.beginPacket();
    LoRa.print("05_Vandalismo");   // Envia que teve vandalismo
    LoRa.endPacket();
    delay(30);
   }
   delay(1000);
...
// MONITORA A CONDICAO SE ABERTO OU FECHADO! **************************************************************************************************************
 ...
 if(digitalRead(entrada_condicao)==LOW && vezes_aberta == 0)
 {
  ...
  {
   LoRa.beginPacket();
   LoRa.print("05_Aberta");   // Envia que esta aberta
   LoRa.endPacket();
   delay(30);
  }
  delay(2000);
 ...
 else if(digitalRead(entrada_condicao)==HIGH && vezes_fechada == 0)
 {
  ...
  {
   LoRa.beginPacket();
   LoRa.print("05_Fechada");   // Envia que esta fechada
   LoRa.endPacket();
   delay(30);
  }
  delay(2000);
