...
#define pino                      5                   //Pino que executara a acao dado no topico "esp8266/pincmd" e terá seu status informado no tópico "esp8266/pinstatus"
...
void retorno(char* topico, byte* mensagem, unsigned int tamanho) {
  ...
  imprimirSerial(true, String(digitalRead(pino)).c_str());
   ...
   // Verifica a mensagem
  if(strMensagem == "liga" && digitalRead(pino)== LOW)
  {
  ...
  else if(strMensagem == "desliga" && digitalRead(pino)== HIGH)
  {
