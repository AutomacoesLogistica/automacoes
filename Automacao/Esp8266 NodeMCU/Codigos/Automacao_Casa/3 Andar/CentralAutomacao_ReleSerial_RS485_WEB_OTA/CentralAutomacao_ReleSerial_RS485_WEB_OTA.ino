/*
  ...
  PARA ACESSAR VIA OTA IP 192.168.20.201
...
String local = "ESP_Central3Andar";
const char* host = "192.168.3.221"; // IP DO SERVIDOR PHP RASPBERRY CENTRAL DA SALA
const int port = 80;
...
IPAddress staticIP(192, 168, 20, 201);
...
EthernetClient ESP_Central3Andar; // Nome de cada Servidor UNICO para cada Arduino
PubSubClient client(ESP_Central3Andar); // Nome de cada Servidor UNICO para cada Arduino


WiFiClient ESP_Central3Andar2;
ESP8266HTTPUpdateServer atualizadorOTA_ESP_Central3Andar2; //Este e o objeto que permite atualizacao do programa via wifi (OTA)
ESP8266WebServer servidorWeb_ESP_Central3Andar2(80); //Servidor Web na porta 80

String dispositivo = "ESP8266 Central Automacao Reles";
...
void ConectarWIFI()
{
 ...
 mac = "x.x.x.x.x.x";//WiFi.macAddress();
 delay(1000);
 Pagina();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  servidorWeb_ESP_Central3Andar2.on("/", ChamPaginaWeb);
}//Fecha void Pagina 
void ChamPaginaWeb() {servidorWeb_ESP_Central3Andar2.send(200, "text/html", paginaWeb);}
...
void chama_atualizar_php(String notificacao)
{
 // Use WiFiClient class to create TCP connections
  /*
 if (!ESP_Central3Andar2.connect(host, port)) {
    Serial.println("Falha na conexao");
    delay(50);
    return;
  }

  String url = "/AutomacaoCasa/salvar_notificacoes.php?";
         url += "notificacao=";
         url += (notificacao);
   ESP_Central3Andar2.print(String("GET ")+ url+ " HTTP/1.1\r\n" + "Host: " + host + "\r\n" + "Connection: close\r\n\r\n");
...
void imprimir()
{
  ...
  if ( readString.indexOf("sala_1") >= 0 || readString.indexOf("sala_0") >= 0 ||  readString.indexOf("teto_sala") >= 0 ||  readString.indexOf("sala4") >= 0)
  {
   sala();
  }
  if ( readString.indexOf("slustre_1") >= 0 || readString.indexOf("slustre_0") >= 0 ||readString.indexOf("lustre_sala") >= 0 ||  readString.indexOf("sala6") >= 0 ||  readString.indexOf("sala7") >= 0)
  {
   sala();  
  }
  if ( readString.indexOf("svaranda_1") >= 0 || readString.indexOf("svaranda_0") >= 0 || readString.indexOf("varan_sala") >= 0 ||  readString.indexOf("sala9") >= 0 ||  readString.indexOf("sala10") >= 0)
  {
    sala();
  }
  if ( readString.indexOf("spainel_1") >= 0 || readString.indexOf("spainel_0") >= 0 || readString.indexOf("amb_sala") >= 0 ||  readString.indexOf("sala5") >= 0)
  {
   sala();
  }
  if ( readString.indexOf("scorti_1") >= 0 || readString.indexOf("scorti_0") >= 0 || readString.indexOf("lam_cor_sala") >= 0 ||  readString.indexOf("sala8") >= 0)
  {
   sala();
  }
  ...
  // COZINHA   **************************************************************************************************************************************************************************
   ...
   if ( readString.indexOf("cozinha_1") >= 0 || readString.indexOf("cozinha_0") >= 0 || readString.indexOf("teto_cozinha") >= 0 ||  readString.indexOf("cozinha4") >= 0)
  {
   cozinha();
  }
  ...
  if ( readString.indexOf("czpendente_1") >= 0 || readString.indexOf("czpendente_0") >= 0 || readString.indexOf("pend_cozinha") >= 0 ||  readString.indexOf("cozinha6") >= 0)
  {
   cozinha(); 
  }
  ...
  //hora para banheiro social
  if ( readString.indexOf("czpersi_1") >= 0 || readString.indexOf("czpersi_0") >= 0 || readString.indexOf("lam_per_coz") >= 0) 
  {
   cozinha(); 
  }
  if ( readString.indexOf("czpainel_1") >= 0 || readString.indexOf("czpainel_0") >= 0 || readString.indexOf("amb_ptv_coz") >= 0 ||  readString.indexOf("cozinha5") >= 0)
  {
   cozinha(); 
  }
  ...
  //Pode ser usada para a iluminação do banheiro suite! MAS AINDA NAO ESTA!
  ...
  if ( readString.indexOf("espgourmet_1") >= 0 || readString.indexOf("espgourmet_0") >= 0 || readString.indexOf("teto_gourmet") >= 0 ||  readString.indexOf("espgour1") >= 0)
  {
   espacogourmet(); 
  }
  if ( readString.indexOf("egpendente_1") >= 0 || readString.indexOf("egpendente_0") >= 0 || readString.indexOf("pend_gourmet") >= 0 ||  readString.indexOf("espgour2") >= 0)
  {
   espacogourmet(); 
  }
  if ( readString.indexOf("egchurras_1") >= 0 || readString.indexOf("egchurras_0") >= 0 || readString.indexOf("chur_gourmet") >= 0 ||  readString.indexOf("espgour3") >= 0)
  {
   espacogourmet(); 
  }
  // HOJE NAO USADO PARA NADA!
  if ( readString.indexOf("egpersi_1") >= 0 || readString.indexOf("egpersi_0") >= 0 || readString.indexOf("lam_per_eg") >= 0) 
  {
   espacogourmet(); 
  }
  //HOJE NAO USADO PARA NADA!
  if ( readString.indexOf("espgourmet2_1") >= 0 || readString.indexOf("espgourmet2_0") >= 0 || readString.indexOf("lam_lav_eg") >= 0 ||  readString.indexOf("espgour4") >= 0)
  {
   espacogourmet(); 
  }
  ...
  // NAO USADAS HOJE - PRETENDO LIGAR!
  ...
  //Ainda nao funcionada! VERIFICAR!
 ...
 {
  chama_atualizar_php(readString); // trato dentro do php e la busca no banco convertendo o payload para mensagem desejada!
 }
 //Publica tambem caso queira realizar automacoes dos interrruptores pelo node red
 //client.publish("dev/test/automacoes/interruptores", readString);
...
void reconectar() {
  ...
  while (!client.connected())
  {
    conectado = strlen(servidor_mqtt_usuario) > 0 ?
                client.connect("ESP_Central3Andar", servidor_mqtt_usuario, servidor_mqtt_senha) :
                client.connect("ESP_Central3Andar");
    if (conectado)
    {
...
void setup()
{
  ...
  Ethernet.begin(mac2,myIP,myDNS,myGW,myMASK);
  RS485.begin(9600);
 ...
 // Iniciar servidor atualizacao OTA
  atualizadorOTA_ESP_Central3Andar2.setup(&servidorWeb_ESP_Central3Andar2);
  servidorWeb_ESP_Central3Andar2.begin();
...
void loop()
{
 ...
 if (readString.length() > 5 && readString.length() < 50)
 {
  imprimir();
 }
  if (!client.connected())
 ...
 {
  servidorWeb_ESP_Central3Andar2.handleClient();
 }
