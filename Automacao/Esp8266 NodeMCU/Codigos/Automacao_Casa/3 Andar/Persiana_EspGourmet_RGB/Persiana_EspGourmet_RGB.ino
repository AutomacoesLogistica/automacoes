...
// MQTT: Topicos
// Estado
const PROGMEM char* MQTT_LIGHT_STATE_TOPIC = "dev/test/minhacasa/espgourmet/persiana/status";
const PROGMEM char* MQTT_LIGHT_COMMAND_TOPIC = "dev/test/minhacasa/espgourmet/persiana/comando";

// Brilho
const PROGMEM char* MQTT_LIGHT_BRIGHTNESS_STATE_TOPIC = "dev/test/minhacasa/espgourmet/persiana/brilho/status";
const PROGMEM char* MQTT_LIGHT_BRIGHTNESS_COMMAND_TOPIC = "dev/test/minhacasa/espgourmet/persiana/brilho/setar";

// Cores(rgb)
const PROGMEM char* MQTT_LIGHT_RGB_STATE_TOPIC = "dev/test/minhacasa/espgourmet/persiana/rgb/status";
const PROGMEM char* MQTT_LIGHT_RGB_COMMAND_TOPIC = "dev/test/minhacasa/espgourmet/persiana/rgb/setar";
...
WiFiClient persiana_espgourmet_rgb;
PubSubClient client(persiana_espgourmet_rgb);
ESP8266HTTPUpdateServer atualizadorOTA_persiana_espgourmet_rgb; //Este e o objeto que permite atualizacao do programa via wifi (OTA)
ESP8266WebServer servidorWeb_persiana_espgourmet_rgb(80); //Servidor Web na porta 80

String dispositivo = "ESP8266 Controlador RGB da Persiana do Espaço Gourmet";
...
// DADOS QUE SERAO EXIBIDOS NO SITE
  ...
  "<body style='margin: 20px; padding: 0px; background-color: #ADD8E6'>"
    ...
    "<h3>MAC: " + mac + "</h3>"
    "</br></br>Dados:"  
    "<h4>Estado: " + lampada + "</h4>"
    "<h4>Cor: " + cor + "&nbsp;&nbsp;&nbsp;<button style='background-color:rgb("+cor+"); width:70px; height: 25px'></button></h4>"
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  servidorWeb_persiana_espgourmet_rgb.on("/", ChamPaginaWeb);
}//Fecha void Pagina 
void ChamPaginaWeb() {servidorWeb_persiana_espgourmet_rgb.send(200, "text/html", paginaWeb);}
...
void publishRGBColor() 
{
 ...
 client.publish("dev/test/minhacasa/espgourmet/persiana/red",m_msg_buffer);//Publica o valor de vermelho
 snprintf(m_msg_buffer, MSG_BUFFER_SIZE, "%d", m_rgb_green);
 client.publish("dev/test/minhacasa/espgourmet/persiana/green",m_msg_buffer);//Publica o valor de verde
 snprintf(m_msg_buffer, MSG_BUFFER_SIZE, "%d", m_rgb_blue);
 client.publish("dev/test/minhacasa/espgourmet/persiana/blue",m_msg_buffer);//Publica o valor de azul
 cor = String(m_rgb_red)+","+String(m_rgb_green)+","+String(m_rgb_blue);
 Pagina();
}
...
void setup() {
  ...
  atualizadorOTA_persiana_espgourmet_rgb.setup(&servidorWeb_persiana_espgourmet_rgb);
  servidorWeb_persiana_espgourmet_rgb.begin();
}
...
void loop() {
 ...
 {
  servidorWeb_persiana_espgourmet_rgb.handleClient();
 }
  client.loop();
}
