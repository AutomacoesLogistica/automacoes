/*
   ...
   MODULO DA ESPACO G IP 192.168.2.207
...
#define LedStatus 16 // LedStatus                      PINO D0
                                            //207
#define MACADDRESS 0x00,0x01,0x02,0x03,0x04,0xCF
#define MYIPADDR 192,168,2,207
...
String ValorIP = "192.168.2.207"; // Colocar o mesmo que ip, este serve para impressao via mqtt
String id = "central_espacog"; // SEMPRE em minusculo
...
EthernetClient Esp8266_MQTT_espacog; // Nome de cada Servidor UNICO para cada Arduino
PubSubClient client(Esp8266_MQTT_espacog); // Nome de cada Servidor UNICO para cada Arduino
...
void atualizar_mensagem(char* topico, byte* mensagem, unsigned int tamanho)
{
  ...
  if (strMensagem == id )
  {
    ...
    client.publish("dev/test/minhacasa/espacog", MensagemRecebida);
    delay(100);
  }
  else
  {
...
void reconectar() {
  ...
  while (!client.connected())
  {
    conectado = strlen(servidor_mqtt_usuario) > 0 ?
                client.connect("Esp8266_MQTT_espacog", servidor_mqtt_usuario, servidor_mqtt_senha) :
                client.connect("Esp8266_MQTT_espacog");
    if (conectado)
    {
