/*
   ...
   MODULO DO QUARTO2 IP 192.168.2.204
...
#define LedStatus 16 // LedStatus                      PINO D0
                                            //204
#define MACADDRESS 0x00,0x01,0x02,0x03,0x04,0xCC
#define MYIPADDR 192,168,2,204
...
String id = "central_quarto2"; // SEMPRE em minusculo
...
EthernetClient Esp8266_MQTT_quarto2; // Nome de cada Servidor UNICO para cada Arduino
PubSubClient client(Esp8266_MQTT_quarto2); // Nome de cada Servidor UNICO para cada Arduino
...
void atualizar_mensagem(char* topico, byte* mensagem, unsigned int tamanho)
{
  ...
  if (strMensagem == id )
  {
    ...
    client.publish("dev/test/minhacasa/quarto2", MensagemRecebida);
    delay(100);
  }
  else
  {
...
void reconectar() {
  ...
  while (!client.connected())
  {
    conectado = strlen(servidor_mqtt_usuario) > 0 ?
                client.connect("Esp8266_MQTT_quarto2", servidor_mqtt_usuario, servidor_mqtt_senha) :
                client.connect("Esp8266_MQTT_quarto2");
    if (conectado)
    {
...
void setup()
{
  ...
  ArduinoOTA.setHostname("central_quarto2"); // nome que ira aparecer na rede
  // No authentication by default
  ArduinoOTA.setPassword((const char *)"268300"); // Senha para permitir atualização
