/*
   ...
   MODULO DA COZINHA IP 192.168.2.205
...
#define LedStatus 16 // LedStatus                      PINO D0
                                            //205
#define MACADDRESS 0x00,0x01,0x02,0x03,0x04,0xCD
#define MYIPADDR 192,168,2,205
...
String ValorIP = "192.168.2.205"; // Colocar o mesmo que ip, este serve para impressao via mqtt
String id = "central_cozinha"; // SEMPRE em minusculo
...
EthernetClient Esp8266_MQTT_cozinha; // Nome de cada Servidor UNICO para cada Arduino
PubSubClient client(Esp8266_MQTT_cozinha); // Nome de cada Servidor UNICO para cada Arduino
...
void atualizar_mensagem(char* topico, byte* mensagem, unsigned int tamanho)
{
  ...
  if (strMensagem == id )
  {
    ...
    client.publish("dev/test/minhacasa/cozinha", MensagemRecebida);
    delay(100);
  }
  else
  {
...
void reconectar() {
  ...
  while (!client.connected())
  {
    conectado = strlen(servidor_mqtt_usuario) > 0 ?
                client.connect("Esp8266_MQTT_cozinha", servidor_mqtt_usuario, servidor_mqtt_senha) :
                client.connect("Esp8266_MQTT_cozinha");
    if (conectado)
    {
