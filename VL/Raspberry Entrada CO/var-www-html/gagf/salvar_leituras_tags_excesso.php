...
$verifica_carreta = (substr($epc,0,6)); // Extrai e verifica se é carreta 442002 se nao sai
...
if ($funcao != "-")
{
 ...
 // Vou deixar salvar mesma coisa ja que a Denise nao alimenta mais os dados
 if ($placa !="0") // Somente se tiver encontrado a placa
 {
  #Salvando os dados dentro do banco de dados ************************************************************************************************
  $sql3 = $dbcon->query("INSERT INTO lista_leitura_tags(ca,data,hora,epc,antena,funcao,placa,localidade,operador)VALUES('$ca','$data','$hora','$epc','$antena','$funcao','-','$localidade', '$operador')");
 }
} // Fecha if $funcao != "-"

if ($verifica_carreta == "442002")
{
 // DEIXA FAZER O QUE ESTA PARA BAIXO, SE NÃO NEM SALVA O EXCESSO, SOMENTE ALEITURA DA TAG
 ...
 if ($excesso == "sim")
 {
  if($local_instalacao == "Miguel Burnier")
  {
   ...
   $result = 0;
   $result2 = 0;
   if ($placa !="0") // Deixa salvar todos ja que a Denise nao controla as tag no meu sistema
   {
   ...
   } // Fecha if de if ($placa != "0")
  }
 } // Fecha o if se $excesso == sim
}
else
{
 // caso nao tenha sido 442002 entra aqui e nao faz nada
}
?>
