...
<?php
$sql = $dbcon->query("SELECT * FROM motivos_acionamentos ORDER BY motivo ASC");
if(mysqli_num_rows($sql)>0)
{
 ?>
...
function salvar_acionamento()
{
    ...
    if(registro.value != "" && senha.value !="" && motivos.value !="")
    {
     
     window.location.href = `consultar_salvar_acionamento.php?complemento=${criptografia2.value}&check=${criptografia.value}&registro=${registro.value}&senha=${senha.value}&motivos=${motivos.value}&cancela=${cancela}&sitio=${sitio}&cod=${cancelaa}&codigo_lora=${codigo_lora}`;  
