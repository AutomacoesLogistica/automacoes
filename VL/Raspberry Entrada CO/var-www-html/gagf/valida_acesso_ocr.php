...
elseif($validado == 2)
{
    echo json_encode('primeiro_acesso'.','.$nome.','.$registro.','.$criptografia);
}
elseif($validado == 3)
{
    echo json_encode('igual,igual,igual,igual'); // Igual é não é o primeiro acesso!
}
elseif($validado == 4)
{
    echo json_encode('nao_cadastrado,nao_cadastrado,nao_cadastrado,nao_cadastrado');
}
else
{
    echo json_encode('negado,negado,negado,negado');
}
