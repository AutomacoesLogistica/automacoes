...
<img id="voltar" src="./images/btn_voltar.png" onclick="javascript: location.href='./dashboard_vl.php';"/>
...
if($nome_coluna == 'Entrada CO' || $nome_coluna == 'Entrada BH')
{
   $nome_coluna = 'Entrada'; 
...
else if($nome_coluna == 'Controles 1, 2 e 3')
{
    $nome_coluna = 'Controle';
...
else if($nome_coluna == 'Excesso')
{
    $nome_coluna = 'Excesso';
    $sql = $dbcon->query("SELECT * FROM limites WHERE referencia='excesso' LIMIT 1");
    if(mysqli_num_rows($sql)>0)
    {
     while($dados = $sql->fetch_array())
     {
