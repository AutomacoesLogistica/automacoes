...
else if(intval($valor_hora)>=17 && intval($valor_hora)<23)
{
  ...
  $turno_atual = $turno3;  
    ...
    else
    {
     ...
     {
      include_once 'conexao_dashboard.php';
      $sql = $dbcon->query("UPDATE dashboard SET tipo='Controle', ponto='Controle 1', data_leitura='$data', hora_leitura='$hora' WHERE id='$id_banco_dashboard'");
     }
     if($encontrados_historico == 1) // Posso atualizar
     {
      ...
      include_once 'conexao_dashboard.php';
      $sql = $dbcon->query("UPDATE historico SET turno='$turno_atual', v_status='Controle Acesso',valor_ponto='$valor_ponto', $ponto='Controle 1', $data_leitura='$data', $hora_leitura='$hora' WHERE id='$id_banco_historico'");
     }
	 ...
	 else
	 {
	  ...
	  include_once 'conexao_dashboard.php';
      $sql = $dbcon->query("INSERT INTO dashboard (epc_cavalo,epc_carreta,placa_cavalo,placa_carreta,tipo,ponto,data_leitura,hora_leitura) VALUES ('-','$tag_carreta','$placa_cavalo','$placa_carreta','Controle', 'Controle 1', '$data', '$hora')");
