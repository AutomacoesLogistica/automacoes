...
<body>
    ...
    error_reporting(0);
     ...
     if($equipamento == '442002')
     {
      ...
      $tag_carreta = $tag;
      $ultima_epc = "";
      //Agora verifico se a ultima tag nao é essa, senao nem salvo
      include_once 'conexao_dashboard.php';
      $sql = $dbcon->query("SELECT * FROM validacoes_tags_tora_fjx ORDER BY id DESC LIMIT 1");
      if(mysqli_num_rows($sql)>0)
      {
       $dados = $sql->fetch_array();
       $ultima_epc = $dados['placa_ou_tag'];
      }
      if($ultima_epc != $tag_carreta)
      {
