...
$sql = $dbcon->query("SELECT * FROM historico_match WHERE (tratado='nao' AND condicao='entrando') LIMIT 1");
$encontrado = 0;
$placa_cavalo = '';
$placa_carreta = '';
...
include_once 'conexao.php';
$sql = $dbcon->query("SELECT * FROM transportadoras WHERE nome='$nome' LIMIT 1");
if(mysqli_num_rows($sql)>0)
{
 $dados = $sql->fetch_array();
 $sigla = $dados['sigla'];
...
include_once 'conexao_automacoes.php';
$sql = $dbcon->query("UPDATE rom SET sigla='$sigla',placa='$placa_carreta' WHERE id='1'");
