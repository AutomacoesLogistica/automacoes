...
if($v_antena == 'sim' && $v_equipamento == 'sim')
{
 ...
 if($condicao == 'Entrando')
 {
  echo '</BR> Entrou para tratar o entrando! </BR>';
  ...
  if($tipo == 'Carreta')
  {
   ...
   $sql = $dbcon->query("SELECT * FROM cheio_vazio WHERE (camera_id='ms0742-cam00' AND placa = '' ) ORDER BY id DESC LIMIT 1"); //Camera entrada
   if(mysqli_num_rows($sql)>0)
   {
    $dados = $sql->fetch_array();
    ...
    $minuto = $mensagem[1];
    $segundo = $mensagem[2];
    if(intval($minuto)<15 && intval($hora)==0 ){$achou = 1;}else{$achou = 0;}
    mysqli_close();
   }
   ...
   else
   {
    ...
    $data = date('d/m/Y');
    $hora = date('H:i:s');
    $sql = $dbcon->query("INSERT INTO leituras_pendentes (epc,placa,gagf,gscs,nomination,material,liquido,resumo_transportadora,condicao,tratado,data_atualizacao,hora_atualizacao) VALUES ('$tag','$placa_salvar','','','','','','$resumo_transportadora','Entrada','nao', '$data', '$hora')"); 
    mysqli_close();
   }
  } // fecha se e carreta
