 ...
 { 
  ...
  if($tamanho_data==10 && $tamanho_hora == 8)
  {
   ...
   else
   {
    ...
    {
     //echo "Tem";echo"</BR>"; echo"</BR>"; echo"</BR>"; echo"</BR>"; echo"</BR>"; 
...
if($encontrado != 0)
{
 ...
 {  
  ...
  include_once 'conexao.php';
  $sql = $dbcon->query("SELECT * FROM historico WHERE (epc_carreta='$array_epc_carreta_saida[$i]' AND v_status ='Saiu da Planta') ORDER BY id DESC LIMIT 1");
  if(mysqli_num_rows($sql)>0)
  {
   ...
   if($id_historico !='')
   {
    ...
    if($v_data_leitura1 !='-')
    {
     ...
     include_once 'conexao.php';
     $sql = $dbcon->query("SELECT * FROM movimentacoes_2024 WHERE mes='$mes_movimentacoes' ORDER BY id ASC LIMIT 1");
     if(mysqli_num_rows($sql)>0)
     {
      ////echo 'entrou';
      $dados = $sql->fetch_array();
     ...
     if($bloqueia_update == 0)
     {
      $sql = $dbcon->query("UPDATE movimentacoes_2024 SET $dia='$quantidade_v_dia', quantidade='$quantidade' WHERE mes='$mes_movimentacoes'");
     }
     if($publica == 1)
     {
      echo'Rodou atualizando movimentacoes_2024 ************************************';
     }
     $turno1 = 'X';
     $turno2 = 'X';
     $turno3 = 'X';
     ...
     $ant_v_turno2 = 0;
     $ant_v_turno3 = 0;
     //Agora atualizo lista_turno_dashboard_2024
     $sql = $dbcon->query("SELECT * FROM lista_turno_dashboard_2024 WHERE data='$v_data_leitura1' ORDER BY id ASC LIMIT 1");
     if(mysqli_num_rows($sql)>0)
     {
      $dados = $sql->fetch_array();
      $turno1 = $dados['turno1'];
      ...
      if($bloqueia_update == 0)
      {
       $sql = $dbcon->query("UPDATE lista_turno_dashboard_2024 SET v_turno1='$v_turno1', v_turno2='$v_turno2', v_turno3='$v_turno3',ant_v_turno1='$ant_v_turno1',ant_v_turno2='$ant_v_turno2',ant_v_turno3='$ant_v_turno3',antena='$ant' WHERE data='$v_data_leitura1'");
      }

      if($publica==1)
      {
       echo ('</BR>Atualizou lista_turno_dashboard_2024 ******************************</BR>');
      }
    ...
    if(($ponto1 == 'Entrada Pires')&& $v_data_leitura1 != '-' && $v_hora_leitura1 != '-')
    {
    ...
    if( ($ponto2 == 'Controle 1'  || $ponto2 == "Controle LE UTMI" || $ponto2 == 'Controle 2' || $ponto2 == "Controle LD UTMI" ) && $v_data_leitura2 != '-'  && $v_hora_leitura2 != '-')
    {
     if($publica==1)
     {
  ...
  if(mysqli_num_rows($sql)>0)
  {
   ...
   if($id_historico !='')
   {
    ...
    if($v_data_leitura1 !='-')
    {
     ...
     if($publica==1)
     {
      echo('Agora atualizo movimentacoes_2024');
     }
     //Agora atualizo lista_turno_dashboard_2024
     $sql = $dbcon->query("SELECT * FROM lista_turno_dashboard_2024 WHERE data='$v_data_leitura1' ORDER BY id ASC LIMIT 1");
     if(mysqli_num_rows($sql)>0)
     {
      ...
      if($bloqueia_update==0)
      {
       $sql = $dbcon->query("UPDATE lista_turno_dashboard_2024 SET v_turno1='$v_turno1', v_turno2='$v_turno2', v_turno3='$v_turno3',job_v_turno1='$job_v_turno1',job_v_turno2='$job_v_turno2',job_v_turno3='$job_v_turno3',job='$job' WHERE data='$v_data_leitura1'");
      }
      if($publica==1)
      {
      echo('Agora atualizo lista_turno_dashboard_2024');
      }
      //Agora atualizao encerramentos
      $sql = $dbcon->query("SELECT * FROM encerramentos WHERE mes='$mes_movimentacoes' ORDER BY id ASC LIMIT 1");
      if(mysqli_num_rows($sql)>0)
