...
<body>
   ...
   echo 'ID banco dashboard : ' . $id_banco_dashboard;
    ...
    //ANTES DE ATUALIZAR SERIA BOM VALIDAR NO GAGF SE JA PESOU POIS AI SERIA SAINDO, CASO CONTRARIO, SERIA EXCESSO! **********************
    //ANTES DE ATUALIZAR SERIA BOM VALIDAR NO GAGF SE JA PESOU POIS AI SERIA SAINDO, CASO CONTRARIO, SERIA EXCESSO! **********************
    //ANTES DE ATUALIZAR SERIA BOM VALIDAR NO GAGF SE JA PESOU POIS AI SERIA SAINDO, CASO CONTRARIO, SERIA EXCESSO! **********************
    //ANTES DE ATUALIZAR SERIA BOM VALIDAR NO GAGF SE JA PESOU POIS AI SERIA SAINDO, CASO CONTRARIO, SERIA EXCESSO! **********************
    ...
    if($encontrados_dashboard == 1) //Posso atualizar 
     {
      include_once 'conexao.php';
      $sql = $dbcon->query("UPDATE dashboard SET tipo='Saida', ponto='$ponto', data_leitura='$data', hora_leitura='$hora' WHERE id='$id_banco_dashboard'");
      //mysqli_close();  
    }
     if($encontrados_historico == 1) // Posso atualizar
     {
      ...
      include_once 'conexao.php';
      $sql = $dbcon->query("UPDATE historico SET encerrado_por='Antena',v_status='Saiu da Planta',valor_ponto='$valor_ponto', $ponto='$ponto', $data_leitura='$data', $hora_leitura='$hora' WHERE id='$id_banco_historico'");
      //mysqli_close(); 

      //Agora insiro na tabela historico_validacoes para ser verificado no gagf
	 ...
	 if($encontrados_historico == 1 || $encontrados_dashboard == 1 )
	 {
		 ...
		 echo '</BR>';
		 include_once 'conexao.php';
		 $sql = $dbcon->query("SELECT * FROM movimentacoes_2024 WHERE mes='$mes' LIMIT 1");
		 if(mysqli_num_rows($sql)>0)
		 {
		  while($dados = $sql->fetch_array())
		  { 
    ...
    */
    ...
    echo ('Valor dia tabela = ' . $valor_dia_tabela);
		include_once 'conexao.php';
		$sql = $dbcon->query("UPDATE movimentacoes_2024 SET quantidade='$n_movimentacoes_mes', $valor_dia_tabela='$n_movimentacoes_dia' WHERE mes='$mes'");
    //mysqli_close();
