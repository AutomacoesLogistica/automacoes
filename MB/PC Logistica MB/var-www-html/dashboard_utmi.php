...
<script type="text/javascript">
   ...
   google.charts.setOnLoadCallback(drawChart);  
    ...
    $sql = $dbcon->query("SELECT * FROM movimentacoes_2023 WHERE id='$vv_mes'");
    if(mysqli_num_rows($sql)>0)
    {
    ...
    include_once 'conexao.php';
    $sql = $dbcon->query("SELECT * FROM lista_turno_dashboard_2023 WHERE data like '%". $filtro . "%'");
    if(mysqli_num_rows($sql)>0)
    {
     while($dados = $sql->fetch_array())
     { 
    ...
    if(intval($encontrado) >0)
    {
     ...
     include_once 'conexao.php';
     $sql = $dbcon->query("UPDATE movimentacoes_2023 SET ttp_mes='$ttp_mes' WHERE mes='$valor_do_mes'");
    }
    ...
    if(intval($encontrado_entrada) >0)
    {
     ...
     include_once 'conexao.php';
     //$sql = $dbcon->query("UPDATE movimentacoes_2023 SET ttp_mes_entrada='$ttp_mes_entrada' WHERE mes='$valor_do_mes'");
    }
    ...
    include_once 'conexao.php';
    $sql = $dbcon->query("SELECT * FROM lista_turno_dashboard_2023 WHERE data='$data' ORDER BY id ASC LIMIT 1");
    if(mysqli_num_rows($sql)>0)
    {
     $dados = $sql->fetch_array();   
  ...
  google.charts.load('current', {'packages':['corechart']});
   ...
   function drawChart() 
   {
     ...
     var data = google.visualization.arrayToDataTable([
      ...
      ["Controle 1", quantidade_ca1, cor_controles1],
      ["Controle 2", quantidade_ca2, cor_controles2],
...
<label id="titulo">Miguel Burnier</label>
