...
$conexao = "conexao_saida_automacoes.php";
...
$curl = curl_init();
  ...
  if($response == '{"errorCode":"java.lang.NullPointerException","errorMsg":"Erro não mapeado"}')
  {
   ...
   $sql = $dbcon->query("UPDATE $tabela SET api_erro = 'Sim', data_tratado='$data', hora_tratado='$hora'  WHERE (epc_carreta='$epc' AND data_tratado = '-')");
  }
  else
  {
...
if($bloqueia_echo == 1)
{
  ...
  include_once $conexao;
  $sql = $dbcon->query("UPDATE $tabela SET api_erro = 'Nao', data_tratado='$data', hora_tratado='$hora' WHERE (epc_carreta='$epc' AND data_tratado = '-')"); 
