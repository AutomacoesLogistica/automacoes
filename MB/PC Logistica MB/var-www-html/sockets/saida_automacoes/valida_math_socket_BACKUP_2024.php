...
function atualiza___historico_display($id,$status_gagf)
{
 echo "</BR>Iniciou a funcao > atualiza___historico_display < </BR>";
 date_default_timezone_set('America/Sao_Paulo');
 $data = date('d/m/Y');
 $hora = date('H:i:s');   
 if($status_gagf == "Validada!")
 {
  //Atualizo que esta tratando!
  include_once 'conexao_saida_automacoes___atualiza___historico_display.php';
  $dbcon->query("UPDATE historico_display_2024 SET condicao1='Concluída!',data_aqui2='$data',hora_aqui2='$hora' WHERE id='$id'");
 }
...
function atualiza___display_balanca1($id,$epc_carreta,$placa_carreta,$epc_cavalo,$placa_cavalo,$crc_display,$mensagem1,$mensagem2,$mensagem3,$mensagem_aux,$ponto)
{
 echo "</BR>Iniciou a funcao > atualiza___display_balanca1 < </BR>";
 if($crc_display!='')
 {
  //Veio para atualizar depois de tratar os dados do display
  //esta tudo ok! $mensagem1 = '___Tenha_uma_boa___';
  echo "</BR>Entrou no if($crc_display!='')";
  echo "</BR>";
  //Agora atualizo o historico
  include_once 'conexao_saida_automacoes___atualiza___display_balanca1.php';
  $sql = $dbcon->query("UPDATE display_balanca1 SET semaforo_entrada='1',semaforo_saida='0',mensagem1='$mensagem1',mensagem2='$mensagem2',mensagem_aux='$mensagem_aux',epc_carreta = '$epc_carreta',ponto='$ponto',opcao='-2'  WHERE id='1'");

  //Agora tenho que atualizar o historico_display
  //FALTANDO ISSOOOOOOOOOO
  include_once 'conexao_saida_automacoes___atualiza___display_balanca1.php';
  $sql = $dbcon->query("UPDATE historico_display_2024 SET concluido='Sim',mensagem='$mensagem1',mensagem2='$mensagem2',crc_display='$crc_display' WHERE id='$id'");
...
function busca_dados_banco($tag)
{
 ...
 if($vtag == '442002')
 {
  $sql = $dbcon->query("SELECT * FROM historico_display_2024 WHERE (epc_carreta='$tag') ORDER BY id DESC LIMIT 1");
 }
 else
 {
  $sql = $dbcon->query("SELECT * FROM historico_display_2024 WHERE (epc_cavalo='$tag') ORDER BY id DESC LIMIT 1");
 }
 ...
 {
  //NAO ACHOU, PERDEU LEITURA NA BALANCA1 **********
  //PRECISO CONSULTAR TUDO NOVAMENTE
  $localEvento = "Miguel Burnier";
   
  echo "</BR>Iniciando consulta FORÇADA no BeTruck com a EPC = ".$tag."</BR>";

  $curl = curl_init();
  curl_setopt_array($curl, array(
   CURLOPT_URL => 'https://api-public-layer.betruck.com.br/v1/viagem/atual?tagVeiculo='.$tag,
   CURLOPT_RETURNTRANSFER => true,
   CURLOPT_ENCODING => '',
   CURLOPT_MAXREDIRS => 10,
   CURLOPT_TIMEOUT => 0,
  ...
  if($valor>0)
  {
   if($tag != "vazio")
   {
    echo "</BR>Preciso salvar a tag no banco pois nao tem no BETRUCK!</BR>";
    //Preciso salvar a tag pois nao encontrada no sistema do BeTruck
    echo "</BR></BR>Agora verifico pelo GAGF!</BR>";
    //AGORA VERIFICO SE ACHO PELO GAGF
    $curl = curl_init();
    curl_setopt_array($curl, array(
     CURLOPT_URL => 'https://gerdauyardserviceda335bbb3.us2.hana.ondemand.com/gerdau-yard-service/rest/schedule/getScheduleDetailByTruck?tagOrPlate='.$tag,
     CURLOPT_RETURNTRANSFER => true,
     CURLOPT_ENCODING => '',
     CURLOPT_MAXREDIRS => 10,
     CURLOPT_TIMEOUT => 0,
    ...
    if($response == '{"errorCode":"java.lang.NullPointerException","errorMsg":"Erro não mapeado"}' || $response == '{"errorCode":"B006.24","errorMsg":"There is no Schedule with this parameters","errorParams":""}')
    {
      $encontrado = 0; //Erro!
      echo "</BR> ERROOOOOO!</BR>";
    }
    else
    {
     echo "</BR></BR>".$response."</BR>";

     echo "</BR>Achou no GAGF e inicio tratativas!</BR>"; 
     ...
     //AGORA SALVO EM HISTORICO DISPLAY
     $concluido = "nao";
     $status_gagf = $statusProcesso;
     $epc_cavalo = '-';
     $epc_carreta = $tag;
     $telefone = "-";
     $transportadora = $n_transportadora;
     $saida = 'Automações Poste';
     ...
     $hora = date('H:i:s');
     include_once 'conexao_saida_automacoes___atualiza___viajem.php';
     echo "</BR>Status GAGF = " . $status_gagf . "</BR>";
     ...
     {
      $sql = $dbcon->query("INSERT INTO historico_display_2024(data_aqui1,hora_aqui1,epc_cavalo,epc_carreta,placa_cavalo,placa_carreta,condicao1,condicao2,motorista,telefone,transportadora,destino,ponto,material,concluido)
      VALUES('$data','$hora','$epc_cavalo','$epc_carreta','$placa_cavalo','$placa_carreta','Validando!','$status_gagf','$nome_completo','$telefone','$transportadora','$destino','$saida','$material','Não')"); 
     }
     
     //Agora faço a consulta novamente acima, mas dessa vez vai achar pois acabamos de cadastrar
     include_once 'conexao_saida_automacoes___busca_dados_banco.php';
     $encontrado = 0; 
     $vtag = substr($tag,0,6);
     if($vtag == '442002')
  ...
  {
   $valor = "Encontrado";
   echo "</BR>Agora vou validar os novos dados!</BR>";
   //echo $response;
   $jsonObj = json_decode($response);
   ...
   //AGORA SALVO EM HISTORICO DISPLAY
   $concluido = "nao";
   $status_gagf = $status_ultima_viagem;
   $epc_cavalo = $tag_do_cavalo;
   $epc_carreta = $tag_da_carreta;
   $placa_cavalo = $placa_do_cavalo;
   $placa_carreta = $placa_da_carreta;
   $nome_completo = $nome_do_motorista;
   $telefone = $numero_do_telefone;
   $transportadora = $transportadora_ultima_viagem;
   $destino = $destino_ultima_viagem;
   $material = $material_ultima_viagem;
   $saida = 'Automações Poste';
   ...
   include_once 'conexao_saida_automacoes___atualiza___viajem.php';
       ...
       // Dados pelo GAGF                                                                       // Dados BETRUCK
   if(($status_gagf == "Finalizada" || $status_gagf == "Finalizada!") || ( ($status_gagf == "Agendada" || $status_gagf=="Carregada") && $veiculo_pesou_ultima_viagem = "Sim"))
   {
    $sql = $dbcon->query("INSERT INTO historico_display_2024(data_aqui1,hora_aqui1,epc_cavalo,epc_carreta,placa_cavalo,placa_carreta,condicao1,condicao2,motorista,telefone,transportadora,destino,ponto,material,concluido,tratado_por_segurpro,tratado_por_ccl)
    VALUES('$data','$hora','$epc_cavalo','$epc_carreta','$placa_cavalo','$placa_carreta','Validada!','$status_gagf','$nome_completo','$telefone','$transportadora','$destino','$saida','$material','-','Não necessario!','Não necessario!')");
   }
...
//***** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA *****
//***** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA *****
//***** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA *****
//***** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA *****
//***** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA ********** INICIA *****
...
if(($v_epc != 'vazio' &&  strlen($v_epc)==24 )) // TAGS RECEBIDAS FORÇADAS MANUALMENTE EM TESTE
{
 //Pode seguir o fluxo
...
{
 if ( (strlen($epc)==24 && (substr($epc,0,6) =="442002" || substr($epc,0,6) =="442001") )|| ( strlen($v_epc)==24 && (substr($v_epc,0,6) =="442002" || substr($v_epc,0,6) =="442001") )  )  // A segunda parte e para ter condicao de publicar manualmente para testar alguma tag
 {
  if($v_epc != 'vazio')
  {
 ...
 echo "</BR></BR></BR>>>>>>>>>>>>>>>>>>>>>>>>>></BR>";
   ...
   if($pode_salvar == "sim")
   {
    ...
    else
    {
     ...
     //Atualizo que esta tratando!
     atualiza___historico_display($id,$status_gagf);
     ...
     //atualiza___display_balanca1($id,$epc_carreta,$placa_carreta,$epc_cavalo,$placa_cavalo,$crc_display,$mensagem1,$mensagem2,$mensagem3,$mensagem_aux,$ponto);
     ...
     else if($status_gagf == "Cancelado" )
     {
      ...
      else
      {
       ...
       if($saida =="MG030" || $saida == "Automações Poste")
       {
        if($saida == "MG030")
        {
        ...
        {
          echo "Saiu pela Saida Automações, considero um possivel alerta de desvio!";
        }
