...
function tags_betruck($epc_betruck)
{
  date_default_timezone_set('America/Sao_Paulo');
  $data = date('d/m/Y');
  $hora = date('H:i:s');
  include_once 'conexao_saida_automacoes_tags_betruck.php';
  $sql = $dbcon->query("INSERT INTO tags_betruck(epc,data,hora)VALUES('$epc_betruck','$data','$hora')");

}
function notificacao_betruck($condicao_betruck,$epc_betruck,$id_gagf_ultima_viagem,$id_betruck_ultima_viagem)
{
...
function atualiza___viajem($concluido1,$tratado_por_segurpro,$tratado_por_ccl,$vconcluido,$crc_display,$mensagem1,$mensagem2,$id_historico_display)
{
 ...
 $sql = $dbcon->query("UPDATE historico_display SET condicao1='$concluido1',tratado_por_segurpro='$tratado_por_segurpro',tratado_por_ccl='$tratado_por_ccl',concluido='$vconcluido',data_aqui1='$data',hora_aqui1='$hora',crc_display='$crc_display',mensagem='$mensagem1',mensagem2='$mensagem2' WHERE id='$id_historico_display'");
    ...
    else
    {
     ...
     if($status_gagf == "Concluído" || $status_gagf == "Saindo da Planta" )
     {
      ...
      if($alerta1 =='Erro' || $alerta1 =='-' || $alerta1 =='' || $alerta1 =='Tudo OK' ) 
      {
       ...
       $vconcluido = "Sim";
       atualiza___viajem($concluido1,$tratado_por_segurpro,$tratado_por_ccl,$vconcluido,$crc_display,$mensagem1,$mensagem2,$id_historico_display);
   
       //Notificação no BeTruck *****************************************************************
       if($valor == "Encontrado")
       {
        notificacao_betruck($condicao_betruck,$epc_betruck,$id_gagf_ultima_viagem,$id_betruck_ultima_viagem);
       }
