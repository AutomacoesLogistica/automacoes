...
if(mysqli_num_rows($sql)>0)
{
  ...
  */
  ...
  { 
   ...
   include_once 'conexao2.php';
   $sql = $dbcon->query("INSERT INTO eventos SET registro='$registro',descricao='$descricao', justificativa='$motivos',cancela='$cancela', data='$data', hora='$hora', nome_colaborador='$usuario', sitio='$sitio', area='$area'");
   ...
   if($mensagem == 'saida_ccl')
   {
    //Salva um erro no sistema da sva para contar saida
    date_default_timezone_set('America/Sao_Paulo');
    $data_agora = date('d/m/Y');
    $hora_agora = date('H:i:s');
    include_once 'conexao_sva.php';
    $sql = $dbcon->query("INSERT INTO cheio_vazio SET tipo_evento='2',media_server_id='ms0742',detected_at='-',data_leitura='$data_agora',hora='$hora_agora',text='SVA não validou saida!',camera_id='ms0742-cam01',caminho='-',caminho_video='-',justificado='SIM',registro='$registro',nome='$usuario',justificativa='$motivos'");
   
    //Agora limpa a notificação na tela de saindo cheio!
   ...
   <script>
    ...
    ?>
     //Faz o get para normalizar!
         //Faz o get para normalizar!
    $.ajax({
       url: 'http://138.0.77.80:5050/AutomacaoGerdau/monitor_rom.php?'+dados,
       type: 'GET',
       dataType: 'html',
