...
<img id="voltar" src="./images/btn_voltar.png" onclick="javascript: location.href=`dashboard_excesso_mb.php?complemento=${criptografia2.value}&check=${criptografia.value}`;"/>
...
<?php
  ...
  $nome_arquivo = "ListaExcessos_MB_".$mes.".csv";


 $titulo = "Filtro da lista de excessos MB referente ao mês de ".$mes;
 ?>
...
include 'conexao.php';
$sql = $dbcon->query("SELECT * FROM lista_excesso_mb WHERE mes='$filtro'  ORDER BY id DESC");
if(mysqli_num_rows($sql)>0)
{
 $encontrado =0;
 while($dados = $sql->fetch_array())
