  ...
  $turno_atual = $turno3;  
    ...
    else
    {
     ...
     if($encontrados_historico == 1) // Posso atualizar
     {
      ...
      include_once 'conexao_dashboard.php';
      $sql = $dbcon->query("UPDATE historico SET turno='$turno_atual', v_status='Controle Acesso',valor_ponto='$valor_ponto', $ponto='Controle 3', $data_leitura='$data', $hora_leitura='$hora' WHERE id='$id_banco_historico'");
      ...
      if($data_leitura1 !='-' && $hora_leitura1 != '-' && $data_leitura2 != '-' && $hora_leitura2 !='-')
      {
	 ...
	 else
	 {
	  ...
	  include_once 'conexao_dashboard.php';
      $sql = $dbcon->query("INSERT INTO dashboard (epc_cavalo,epc_carreta,placa_cavalo,placa_carreta,tipo,ponto,data_leitura,hora_leitura) VALUES ('-','$tag_carreta','$placa_cavalo','$placa_carreta','Controle', 'Controle 3', '$data', '$hora')");
...
else if(intval($valor_hora)>=17 && intval($valor_hora)<23)
{
  ...
  $ch = curl_init();
  curl_setopt($ch,CURLOPT_URL, 'http://192.168.10.96/dips/verifica_tempo.php');
