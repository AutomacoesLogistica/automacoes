...
function descartar_placa(link_video)
{
...
?>
<label id='cheio_vazio_titulo'>Cheio / Vazio + OCR MB - Eventos do dia  <?php print $data ?></label>
<script>
...
<label id='cheio_vazio_titulo2'> Ultima detecção: <?php print $ultima_data ?></label>
     ...
     <table id='tabela_cheio_vazio'>
        ...
        <tbody id='t_cheio_vazio'>
            ...
            $sql = $dbcon->query("SELECT * FROM deteccao WHERE ".$pesquisa." AND (deteccao_falsa != 'SIM' AND deteccao_falsa != 'BLK')  ORDER BY id DESC");
            if(mysqli_num_rows($sql)>0)
            {
             while($dados = $sql->fetch_array())
             {
              ...
              if($ip == "192.168.10.1") // Entrei localmente!
              {
               //echo('remoto');
              ...
              else
              {
               ...
               $url_do_arquivo = str_replace(".com.",".10.",$url_do_arquivo);
               $url_do_arquivo = str_replace(".br:",".95:",$url_do_arquivo);
              ...
              <tr>
                <td class="cheio_vazio_th1">
                  <div id='foto_cheio_vazio'>
                      ...
                      <label><b>Saida Detectada: </b><?php print $saida_detectada ?></label>  
                      </br>
                      <label><b>Condição: </b><?php print $status_evento?></label>
                      
                      
                    </br>
                      <a href="javascript:cheio_vazio_altera_video('<?php echo $url_do_arquivo ?>');" id='assistir'>Báscula</a>
                      <a href="javascript:cheio_vazio_altera_video('<?php echo $url_do_arquivo2 ?>');" id='assistir'>Placa</a>
                      <?php
                      if($justificado == "SIM")
                      {
                    ...
                    &nbsp&nbsp<a href="javascript:descartar_placa('<?php echo $id ?>');" style="background-color:#FF4500;" id='assistir'>Descartar Placa</a>  
                    </br></br>
                    <label><b>-----------------------------------------------------------------</label>
                  </div>
...
function clicou_salvar_filtro_pessoas()
{
 ...
 location.href=`tela_va_ocr_cheio_vazio.php?complemento=${criptografia2.value}&check=${criptografia.value}&data=`+link_data+`&complemento=`+link_complemento+`&check=`+link_check+`&status=`+resp_cheio_vazio+`&cameras=`+resp_cameras+`&cameras_pessoas=`+resp_cameras_pessoas+`&parecer=`+resp_parecer+`&parecer_pessoas=`+resp_parecer_pessoas;
