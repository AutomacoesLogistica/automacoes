...
<label id="lb_id_lidar" hidden="hidden"></label>
...
?>
<script>
var complemento = parseInt('<?php print $complemento ?>');
if(complemento>0)
{
//alert(complemento);
}
...
$limite = isset($_GET['limite'])?$_GET['limite']:"10000";
...
$resumido = isset($_GET['resumido'])?$_GET['resumido']:"nao";
...
function lidar_altera_video(link_video,id_cheio_vazio)
{
 //alert("Serve para exibir a imagem da báscula relacionada no sistema do lidar! \n\n EM DESENVOLVIMENTO! \n\n ID Video: "+link_video);
 var link_lb_id_lidar = document.getElementById('lb_id_lidar');
 if(link_video != '')
 {
  link_lb_id_lidar.innerHTML = link_video;
  //alert(link_lb_id_lidar.innerHTML);
  $("#v_exibir_lidar").load("div_v_exibir_lidar.php?id="+link_lb_id_lidar.innerHTML+'&id_cheio_vazio='+id_cheio_vazio);
  clicou_exibir_lidar(link_lb_id_lidar.innerHTML);
 }
 else
 {
  alert("Não identificado o valor do ID da imagem LIDAR!\nTente novamente!");
 }
}
...
<label id='cheio_vazio_info_eventos10'>Macth / Tag: 0 </label>
...
<label id='cheio_vazio_titulo3'> Ultimo MATCH: <?php print $ultimo_match ?></label>
     <table id='tabela_cheio_vazio'>
        ...
        <tbody id='t_cheio_vazio'>
            ...
            if($modo == "1") //detectar somente cheio ou vazio
		      	{
			       $pesquisa = $pesquisa .  " AND saida_detectada = 'Saida Principal'";
			      }
            ...
            $sql = $dbcon->query("SELECT * FROM deteccao WHERE ".$pesquisa." AND (deteccao_falsa != 'SIM')  ORDER BY id DESC LIMIT $limite");
            if(mysqli_num_rows($sql)>0)
            {
             while($dados = $sql->fetch_array())
             {
              ...
              if($status_evento == "SUP Superior" || $status_evento == "SUP superior")
              {
                $status_evento = 'Detectado Superior';
              }
			  $veiculo = $dados['veiculo'];
			  if($veiculo == '' || $veiculo ==' ' || $veiculo =='-'){$veiculo = 'Não identificado!';}
			  
			  $condicao_lidar = $dados['condicao_lidar'];
			  $alerta2 = $dados['alerta2'];
			  $id_lidar = $dados['id_lidar'];
			  if($id_lidar =='' || $id_lidar==' ' || $id_lidar=='-'){$id_lidar='Erro';}
			  if($alerta2 =='' || $alerta2 ==' ' || $alerta2 == '-'){$alerta2 = 'Não identificado!';}
			  $dlc = $dados['dlc'];
			  $dtc = $dados['dtc'];
			  if($dlc==''|| $dlc==' '){$dlc='-';}
			  if($dtc==''|| $dtc==' '){$dtc='-';}
			  if($condicao_lidar == '' || $condicao_lidar == ' ' || $condicao_lidar == '-'){$condicao_lidar = '(L) Não identificado!';}
			  if($condicao_lidar =='Cheia'){$condicao_lidar = '(L) Saindo Cheia!';}
			  if($condicao_lidar =='Vazia'){$condicao_lidar = '(L) Saindo Vazia!';}
              ...
              if($ip == "192.168.10.1") // Entrei remoto!
              {
               //echo('remoto');
              ...
              else
              {
               ...
               $url_do_arquivo = str_replace(":3500",":8597",$url_do_arquivo);
			   
			   $url_do_arquivo2 = str_replace("/gerdau1.","/192.",$url_do_arquivo2);
               $url_do_arquivo2 = str_replace(".svatech.",".168.",$url_do_arquivo2);
               $url_do_arquivo2 = str_replace(".com.",".10.",$url_do_arquivo2);
               $url_do_arquivo2 = str_replace(".br:",".95:",$url_do_arquivo2);
               $url_do_arquivo2 = str_replace(":3500",":8597",$url_do_arquivo2);
              }
        ...
        $placa_cavalo = $dados['placa_cavalo'];
			  if($epc_carreta =='' || $epc_carreta== ' ' || $epc_carreta == '-'){$epc_carreta = 'Não identificado!';}
			  if($placa_carreta =='' || $placa_carreta ==' ' || $placa_carreta=='-'){$placa_carreta = '-';}
              ...
              <tr>
                <td class="cheio_vazio_th1">
                  <div id='foto_cheio_vazio'>
                      ...
                      <label><b>Condição: </b><?php print $status_evento ." / ".$condicao_lidar  ?></label>
					  </br>
                      <label><b>Cav.: </b><?php print $tag?></label> / 
					  <label><b>P.: </b><?php print $placa_cavalo ?></label>
					  </br>
                      <label><b>Car: </b><?php print $epc_carreta?></label> / 
					  <label><b>P.: </b><?php print $placa_carreta ?></label>
					  </br>
                      <label><b>DLC: </b><?php print $dlc?></label>&nbsp&nbsp&nbsp&nbsp
					  <label><b>DTC: </b><?php print $dtc?></label>&nbsp&nbsp&nbsp&nbsp&nbsp
					  <label><b>Veiculo: </b><?php print $veiculo?></label>
                      </br>
                      <label><b>Lidar: </b><?php print $alerta2 ?></label>
					...
					{
					?>
					<a href="javascript:lidar_altera_video('<?php echo $id_lidar ?>','<?php echo $id ?>');" style="background-color:#4169E1;" id='assistir'>Imagem Lidar</a>
                    &nbsp&nbsp
					<?php
					}
					?>
					<a href="javascript:descartar_placa('<?php echo $id ?>');" style="background-color:#FF4500;" id='assistir'>Descartar Placa</a>  
                    </br></br>
                    <label><b>-----------------------------------------------------------------</label>
                    </br>
                  </div>
        ...
        //Assertividade match
        n_assertividade_match = (n_match/n_encontrado * 100).toFixed(1) + "%";
        
        
        //Assertividade TAGs
        n_assertividade_tag = (n_tag/n_encontrado * 100).toFixed(1) + "%";

        //Assertividade Match/Tag
        n_assertividade_match_tag = (n_match/n_tag * 100).toFixed(1) + "%";
				...
				document.getElementById('cheio_vazio_info_eventos5').innerHTML = 'Match : '+ n_match+ ' / '+ n_assertividade_match;
				...
				document.getElementById('cheio_vazio_info_eventos9').innerHTML = 'Tags : '+ n_tag + ' / '+ n_assertividade_tag;
				document.getElementById('cheio_vazio_info_eventos10').innerHTML =  'Match/Tag : '+ n_assertividade_match_tag;
...
<label id='outros_veiculos_titulo2'> Ultima detecção: <?php print $ultima_data ?></label>
     ...
     <table id='tabela_outros_veiculos'>
        ...
        <tbody id='t_outros_veiculos'>
            ...
            include_once 'conexao_sva_cheio_vazio.php';
            //$sql = $dbcon->query("SELECT * FROM deteccoes WHERE ".$pesquisa." AND deteccao_falsa != 'SIM' ORDER BY id DESC");
            $sql = $dbcon->query("SELECT * FROM deteccao_proibidas WHERE ".$pesquisa." AND (deteccao_falsa != 'SIM' AND deteccao_falsa != 'BLK') ORDER BY id DESC LIMIT $limite");
            if(mysqli_num_rows($sql)>0)
            {
             while($dados = $sql->fetch_array())
             {
              ...
              else
              {
               ...
               $url_do_arquivo = str_replace(".com.",".10.",$url_do_arquivo);
               $url_do_arquivo = str_replace(".br:",".95:",$url_do_arquivo);
               $url_do_arquivo = str_replace(":3499",":8594",$url_do_arquivo);
              }
...
function clicou_cheio_vazio()
{
    ...
    document.getElementById('v_exibir_lidar').style.display = 'none';
...
function clicou_exibir_lidar(lb_id_lidar)
{
    ...
    document.getElementById('v_outros_veiculos').style.display = 'none';
    document.getElementById('v_radar').style.display = 'none';
    document.getElementById('v_exibir_lidar').style.display = 'block';
    document.getElementById('v_dashboard').style.display = 'none';
    document.getElementById('v_configuracoes').style.display = 'none';
    document.getElementById('v_descartados').style.display = 'none';
    document.getElementById('v_sirene').style.display = 'none';

    //$("#v_exibir_lidar").load("tela_deteccoes_cheio_vazio_mb.php");
    //Oculta
...
LABEL#cheio_vazio_info_eventos10{
    margin-left: 0%;
    position: absolute;
    font: normal 12pt verdana;
    color:#ffffff;
