...
$epc_excesso = $epc;
  ...
  if($epc != 'vazio')
  {
   ...
   if($v1_epc == '442002')
   {
    ...
    //Salvo no banco de dados historico_recebido_python para saber se o python esta rodando corretamente
    echo '</BR>';
    ...
    $hora = date('H:i:s');
    include_once 'conexao.php';
    $sql = $dbcon->query("INSERT INTO historico_recebido_python(epc,ponto,data,hora)VALUES('$epc','Saida Automações','$data','$hora')");
   } 
   ...
   if($v1_epc == '442002')
   {
    ...
    if(mysqli_num_rows($sql)>0)
    {
     ...
     $id_match = $dados['id'];
     $tradado = $dados['concluido'];
     if($tradado == 'sim' || $tratado == 'Sim')
     {
      $encontrado_tratado = 2;//Nao faz nada pois ja foi feito!
     }
     else if($tratado == 'nao' || $tratado == 'Nao' )
     {
    ...
    {
      //NEM ACHOU A TAG, TENHO QUE FAZER ALGO POIS NAO FOI LIDA ANTERIORMENTE NA MG030 OU NA BALANCA
   ...
   elseif($encontrado_tratado==1)
   {
    ...
    include_once 'conexao.php';
    $sql = $dbcon->query("SELECT * FROM historico_display WHERE ( $valor='$epc' AND concluido='nao' ) ORDER BY id DESC LIMIT 1");
    if(mysqli_num_rows($sql)>0)
    {
     $dados = $sql->fetch_array(); 
    ...
    if($condicao2 == 'Concluído' || $condicao2 == 'Saindo da Planta')
    {
     ...
     else
     {
      ...
      include_once 'conexao.php';
      $sql = $dbcon->query("UPDATE historico_display SET condicao1='Concluido2',tratado_por_segurpro='Não necessário!',tratado_por_ccl='Não necessário!',concluido='Sim',data_aqui1='$data',hora_aqui1='$hora',crc_display='$crc_display',mensagem='$mensagem',mensagem2='$mensagem2' WHERE id='$id_historico_display'");
      ...
      else
      {
       ...
       if($epc_excesso != 'vazio' )
       {
         ...
         */  
         ...
         include_once 'conexao.php';
         $sql = $dbcon->query("INSERT INTO lidar_excesso(epc_lidar,condicao,tratado)VALUES('$epc_excesso','Carga Descentralizada!','nao')");
     ...
     echo '</BR>';
     echo 'Condicao diferente de Concluiu ou saindo da planta!';
     echo '</BR>';
     //Atualizo os dados no display_balanca1 
     include_once 'conexao.php';
     $sql = $dbcon->query("UPDATE display_balanca1 SET id_historico='$id_historico_display',epc_cavalo = '$epc_cavalo',placa_cavalo='$placa_cavalo' WHERE id='1'");
      ...
      else
      {
        ...
        if($epc_excesso != 'vazio' )
        {
          ...
          */  
          ...
          //$sql = $dbcon->query("INSERT INTO lidar_excesso(id_lidar,id_cheio_vazio,id_historico,epc_lidar,placa,veiculo,data_leitura,dia,mes,ano,hora_leitura,condicao,tratado,data_tratado,hora_tratado,confirmacao,tempo_confirmacao,motivo)VALUES('$id_lidar','$id_cheio_vazio','$id_historico','$epc_excesso','$placa','$veiculo','$data','$dia','$mes','$ano','$hora','$condicao','nao','-','-','nao','0','-')");
          include_once 'conexao.php';
          $sql = $dbcon->query("INSERT INTO lidar_excesso(epc_lidar,condicao,tratado)VALUES('$epc_excesso','Excesso/Falta','nao')");
