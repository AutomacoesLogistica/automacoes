...
<link rel="shortcut icon" type="imagem/JPG" id="favicon">
...
<label id='lb_hora_display'>--</label><br>

<div id='base_configuracoes' name='base_configuracoes' >
<label id="lb_filtro1">Efetue as configurações do sistema</label>
<label id="lb_filtro2">Configurações API:</label>
<input type="checkbox" id="ckb_atualizar_display" name="ckb_atualizar_display" checked>
<label id="lb_atualizar_display">Atualizar Display</label>
<input type="checkbox" id="ckb_consultar_gagf" name="ckb_consultar_gagf" checked>
<div id='complemento_display'>
<input type="radio" id="rd_apenas_placas" name="fav_language" value="Apenas Placas">
<label id='lb_apenas_placas' for="apenas_placas">Apenas Placas</label><br>
<input type="radio" id="rd_automacao_completa" name="fav_language" value="Automação Completa">
<label id='lb_automacao_completa' for="automacao_completa">Automação Completa</label><br>
</div>
<label id="lb_consultar_gagf">Consultar API GAGF</label>
<input id="txt_msg" type="text"></input>
<input id="btn_testar" type="button" value="Testar" onclick='enviar_teste()' />
<input id="btn_limpar" type="button" value="Limpar Display" onclick='limpar()' />
<input id="btn_salvar" type="button" value="Salvar Configurações" onclick='salvar()' />
<input id="btn_foto_saida_balanca" type="button" value="Bloquear/Desbloquear Semaforos" onclick='semaforos()' />
</div>




<div id='hora_display' name='hora_display' >

<?php
include_once 'conexao.php';
$nome = 'atualiza_display.service';
$sql = $dbcon->query("SELECT * FROM atualizacao_services WHERE nome_service='$nome'");
if(mysqli_num_rows($sql)>0)
{
 $dados = $sql->fetch_array();
 $data_banco1 = $dados['data_atualizacao'];
 $hora_banco1 = $dados['hora_atualizacao'];
}

?>
<script>
    var vv = '<?php print $data_banco1 . ' ' . $hora_banco1 ?>';
document.getElementById('lb_hora_display').innerHTML=vv;
console.log(vv);
</script>
</div>
...
<div id='base_display' name='base_display'>





<div id='base_display2' name='base_display2' >
...
{
 $dados = $sql->fetch_array();
 $excesso = $dados['excesso'];
 $carga_torta = $dados['carga_torta'];
 $roubo_carga = $dados['roubo_carga'];
 $modo_operacao_display = $dados['modo_operacao_display'];
...
?>
<script>
 var link_modo_operacao_display = '<?php print $modo_operacao_display ?>';
 if(link_modo_operacao_display == 'apenas_placas')
 {
    document.getElementById('rd_apenas_placas').checked = true;
    document.getElementById('rd_automacao_completa').checked = false;
 }
 else
 {
    document.getElementById('rd_apenas_placas').checked = false;
    document.getElementById('rd_automacao_completa').checked = true;
 }

 var link_publicar_display = '<?php print $publicar_display ?>';
if(link_publicar_display == 'true')
{
 document.getElementById('ckb_atualizar_display').checked = true;
}
else
{
 document.getElementById('ckb_atualizar_display').checked = false;
}

var link_publicar_gagf = '<?php print $publicar_gagf ?>';
if(link_publicar_gagf == 'sim')
{
 document.getElementById('ckb_consultar_gagf').checked = true;
}
else
{
 document.getElementById('ckb_consultar_gagf').checked = false;
}
</script>
...
if($semaforo_entrada =='0')
{
 $semaforo_entrada = './images/semaforo_verde.png';   
}
elseif($semaforo_entrada =='1')
{
 $semaforo_entrada = './images/semaforo_vermelho.png';   
}
else
{
 $semaforo_entrada = './images/semaforo_apagado.png';   
}
if($semaforo_saida =='0')
{
 $semaforo_saida = './images/semaforo_verde.png';   
}
elseif($semaforo_saida =='1')
{
 $semaforo_saida = './images/semaforo_vermelho.png';   
}
else
{
 $semaforo_saida = './images/semaforo_apagado.png';   
}
?>
<img id="semaforo_entrada" src="<?php print $semaforo_entrada ?>"/>
<img id="semaforo_saida" src="<?php print $semaforo_saida ?>"/>
<?php
...
{
 $mensagem =  "  Balanca 01 - MB   ";
 $mensagem2 = "   Miguel Burnier   ";
}
else
{
 if($mensagem == "")
 ...
 if($mensagem2 == "" || $mensagem2 == "horario")
 {
  $mensagem2 = "                    ";   
 }   
}
...
if($tamanho_mesagem1 != 20)
{
////echo 'menor';
$vezes = 20-intval($tamanho_mesagem1);
for ($i = 0; $i<$vezes;$i++) 
{
...
if($tamanho_mesagem2 != 20)
{
////echo 'menor';
$vezes2 = 20-intval($tamanho_mesagem2);
for ($i = 0; $i<$vezes2;$i++) 
{
...
$linha_1_primeiro = substr($mensagem,0,1);
...
else{$linha_1_primeiro = substr($mensagem,0,1);}

$linha_2_primeiro = substr($mensagem,1,1);
...
else{$linha_2_primeiro = substr($mensagem,1,1);}


$linha_3_primeiro = substr($mensagem,2,1);
...
else{$linha_3_primeiro = substr($mensagem,2,1);}


$linha_4_primeiro = substr($mensagem,3,1);
...
else{$linha_4_primeiro = substr($mensagem,3,1);}


$linha_5_primeiro = substr($mensagem,4,1);
...
else{$linha_5_primeiro = substr($mensagem,4,1);}

$linha_6_primeiro = substr($mensagem,5,1);
...
else{$linha_6_primeiro = substr($mensagem,5,1);}

$linha_7_primeiro = substr($mensagem,6,1);
...
else{$linha_7_primeiro = substr($mensagem,6,1);}

$linha_8_primeiro = substr($mensagem,7,1);
...
else{$linha_8_primeiro = substr($mensagem,7,1);}

$linha_9_primeiro = substr($mensagem,8,1);
...
else{$linha_9_primeiro = substr($mensagem,8,1);}

$linha_10_primeiro = substr($mensagem,9,1);
...
else{$linha_10_primeiro = substr($mensagem,9,1);}

$linha_11_primeiro = substr($mensagem,10,1);
...
else{$linha_11_primeiro = substr($mensagem,10,1);}

$linha_12_primeiro = substr($mensagem,11,1);
...
else{$linha_12_primeiro = substr($mensagem,11,1);}

$linha_13_primeiro = substr($mensagem,12,1);
...
else{$linha_13_primeiro = substr($mensagem,12,1);}

$linha_14_primeiro = substr($mensagem,13,1);
...
else{$linha_14_primeiro = substr($mensagem,13,1);}

$linha_15_primeiro = substr($mensagem,14,1);
...
else{$linha_15_primeiro = substr($mensagem,14,1);}

$linha_16_primeiro = substr($mensagem,15,1);
...
else{$linha_16_primeiro = substr($mensagem,15,1);}

$linha_17_primeiro = substr($mensagem,16,1);
...
else{$linha_17_primeiro = substr($mensagem,16,1);}

$linha_18_primeiro = substr($mensagem,17,1);
...
else{$linha_18_primeiro = substr($mensagem,17,1);}

$linha_19_primeiro = substr($mensagem,18,1);
...
else{$linha_19_primeiro = substr($mensagem,18,1);}


$linha_20_primeiro = substr($mensagem,19,1);
...
else{$linha_20_primeiro = substr($mensagem,19,1);}
...
<div id="aux_semaforo_entrada" hidden='hidden'></div>
<div id="semaforo_entrada" hidden='hidden'>
<img class="semaforos" src=<?php print $caminho.$semaforo_entrada.".png"?> id="can2_pat" onclick="javascript: location.href=`display_mb.php?complemento=${criptografia2.value}&check=${criptografia.value}`;"/>
</div>
<div id="aux_semaforo_saida"  hidden='hidden'></div>
<div id="semaforo_saida" hidden='hidden'>
...
<table border= 2px; >
  <thead >
    <tr>
      ...
      <th class="th4_1">Ponto Saida</th>
      <th class="th5_1">Condição</th>
      <th class="th6_1">SegurPro</th>
      <th class="th7_1">CCL</th>
      <th class="th8_1">GAGF</th>
      <th class="th9_1">GSCS</th>
      <th class="th10_1">Material</th>
      <th class="th11_1">Destino</th>
      <th class="th12_1">Motorista</th>
...
$array_encerrar_job=array();



$sql = $dbcon->query("SELECT * FROM historico_display ORDER BY id DESC LIMIT 6");
if(mysqli_num_rows($sql)>0)
{
 //limpar o array
 unset($array_encerrar_job);
 while($dados = $sql->fetch_array())
 {
  ...
  //verifico se a hora esta ok e tem mais de 3 min
  ...
  if($tamanho_data==10 && $tamanho_hora == 8)
  {
   ...
   //Agora verifico se tem 3 minutos ou mais
   ...
   $minuto = $mensagem[1];
   $segundo = $mensagem[2];
   //coloco na lista para verificar
   $tempo = intval($minuto); // tempo em
   $info = $dados['id'].';'.$tempo.';'.$v_condicao;
   $array_encerrar_job[$encontrado]= $info;
  } 
  ...
  if($v_condicao == '')
  {
   $array_condicao[$encontrado] ='Aguardando';
  }
  else
  {
   if($v_condicao == 'Concluido2')
   {
    $array_condicao[$encontrado] ='Saindo Vazio!';
   }
   else
   {
  ...
  $v_condicao2 = $dados['condicao2'];
  
  if($v_condicao2 == "Concluído" &&  $concluido == "sim")
  {
    $array_condicao[$encontrado] = "Concluído";
  }
  ...
  $v_gagf = $dados['gagf'];
  if($v_gagf == ''){$array_gagf[$encontrado] ='--';}else{$array_gagf[$encontrado] =$v_gagf;}

  $v_gscs = $dados['gscs'];
  if($v_gscs == ''){$array_gscs[$encontrado] ='--';}else{$array_gscs[$encontrado] =$v_gscs;}

  $v_destino = $dados['destino'];
  if($v_destino == ''){$array_destino[$encontrado] ='--';}else{$array_destino[$encontrado] =$v_destino;}

  $v_motorista = $dados['motorista'];
  if($v_motorista == ''){$array_motorista[$encontrado] ='--';}else{$array_motorista[$encontrado] =$v_motorista;}
 
  $v_material = $dados['material'];
  if($v_material == ''){$array_material[$encontrado] ='--';}else{$array_material[$encontrado] =$v_material;}
  
  ?>
    <tr>
    	<td class="th1"><?php print $array_data_hora[$encontrado]?></td>
        <td class="th2"><img id="img_fundo_placas" src="./images/placas/placa_fundo.png"/>
        <div id='v_placa_cavalo'>
        <?php print $array_placa_cavalo[$encontrado]?>
        </div>
        </td>
        <td class="th3"><img id="img_fundo_placas" src="./images/placas/placa_fundo.png"/>
        <div id='v_placa_carreta'>
        <?php print $array_placa_carreta[$encontrado]?>
        </div>
        </td>
        <td class="th4"><?php print $array_ponto[$encontrado]?></td>
        <td class="th5"><?php print $array_condicao[$encontrado]?></td>
        <td class="th6"><?php print $array_tratado_por_segurpro[$encontrado]?></td>
        <td class="th7"><?php print $array_tratado_por_ccl[$encontrado]?></td>
        <td class="th8"><?php print $array_gagf[$encontrado]?></td>
        <td class="th9"><?php print $array_gscs[$encontrado]?></td>
        <td class="th10"><?php print $array_material[$encontrado]?></td>
        <td class="th11"><?php print $array_destino[$encontrado]?></td>
        <td class="th12"><?php print $array_motorista[$encontrado]?></td>
...
if($array_condicao[5]=='Aguardando')
{
 //Primeiro vejo se ja nao esta na lista
 include_once 'conexao.php';
 $sql = $dbcon->query("SELECT * FROM pendente_saida_automacoes WHERE id_historico = '$array_id[5]' LIMIT 1");
 if(mysqli_num_rows($sql)>0)
 {
  $dados = $sql->fetch_array();
 }
 ...
 {
  //Salvo na tabela para tratar por la e encerrar essa viagem!
  date_default_timezone_set('America/Sao_Paulo');
  $data = date('d/m/Y');
  $hora = date('H:i:s');
  include_once 'conexao.php';
  $sql = $dbcon->query("INSERT INTO pendente_saida_automacoes(id_historico,data_leitura,hora_leitura,condicao,data_tratado,hora_tratado)VALUES('$array_id[5]','$data','$hora','pendente','-','-')");
 }
}



if($array_condicao[6]=='Aguardando')
{
 //Primeiro vejo se ja nao esta na lista
 include_once 'conexao.php';
 $sql = $dbcon->query("SELECT * FROM pendente_saida_automacoes WHERE id_historico = '$array_id[6]' LIMIT 1");
 if(mysqli_num_rows($sql)>0)
 {
  $dados = $sql->fetch_array();
 }
 else
 {
  ...
  $hora = date('H:i:s');
  include_once 'conexao.php';
  $sql = $dbcon->query("INSERT INTO pendente_saida_automacoes(id_historico,data_leitura,hora_leitura,condicao,data_tratado,hora_tratado)VALUES('$array_id[6]','$data','$hora','pendente','-','-')");
 }
}
...
$tamanho = count($array_encerrar_job);
?>
<script>
console.log('Tamanho = ' + '<?php print $tamanho ?>');
</script>
<?php
for ($x = 1; $x <= $tamanho; $x++) 
{
 $msg = explode(';',$array_encerrar_job[$x]);
 $v_id = $msg[0];
 $v_tempo =  $msg[1];
 $a_condicao = $msg[2];
 if($a_condicao == "Aguardando" && intval($v_tempo)>=5)
 {
  //encerra via job
  ?>
  <script>
  v_id = '<?php print $v_id ?>';
  console.log("Encerrando via JOB o id " + v_id)   ;
  $.ajax({
         url: 'job_saida_automacoes.php',
         type: 'GET',
         dataType: 'text',
         data: {'id': v_id},
         success: function(resultado)
         {
          ...
          //alert('O sistema acabou de ser ' + resultado);
 ...
 else if($a_condicao == "Aguardando" && intval($v_tempo)<5)
 {
  //encerra via job
  ?>
  <script>
  v_tempo = '<?php print $v_tempo ?>';
  console.log("Aguardando o tempo para encerrar via JOB, tempo de: " + v_tempo)   ;
  </script>
  <?php  
  }
...
$(document).ready(function(){
setInterval(function(){
     $("#base_display2").load(window.location.href + " #base_display2");
    
}, 800);
});
//Atualiza hora do servico do display
$(document).ready(function(){
setInterval(function(){
     $("#hora_display").load(window.location.href + " #hora_display");
    
}, 1000);
});


function recarregar_configuracoes()
{
 //$("#base_configuracoes").load(window.location.href + " #base_configuracoes");
}
...
INPUT#btn_foto_saida_balanca:hover
{
 background-color: #555555; /* Preto */
 color: white;
}
INPUT#btn_foto_saida_balanca:active {
...
INPUT#btn_testar:hover
{
 background-color: #555555; /* Preto */
 color: white;
}
INPUT#btn_testar:active {
  background-color: #00008B;
  box-shadow: 0 5px #666;
  transform: translateY(4px);
}
...
INPUT#btn_limpar:hover
{
 background-color: #555555; /* Preto */
 color: white;
}
INPUT#btn_limpar:active {
  background-color: #00008B;
  box-shadow: 0 5px #666;
  transform: translateY(4px);
}
