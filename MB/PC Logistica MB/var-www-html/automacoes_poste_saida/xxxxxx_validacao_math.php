<?php
$bloqueia_atuacoes_banco = isset($_GET['bloqueia'])?$_GET['bloqueia']:0;

$ponto = isset($_GET['ponto'])?$_GET['ponto']:"vazio";
$epc1 = isset($_GET['epc'])?$_GET['epc']:"vazio";
$encontrado = 0;
$placaCarreta = '';
$placaCavalo = '';
$epc_carreta = '-';
...
if($ponto != "vazio")
{
    ...
    if($encontrado == 1 && strlen($epc_carreta)==24 )
    {
     ...
     } //Fecha if($response == '{"errorCode":"java.lang.NullPointerException","errorMsg":"Erro não mapeado"}')
     else
     {
     ...
     if($encontrado_tag == 1)
     {
      ...
      echo "</BR>Ja existe a TAG do cavalo, não preciso adiciona-la!.....Tag: ".$epc_cavalo."</BR>";
     } // Fecha if($encontrado_tag == 1)
     else
     {
     ...
     else
     {
      ...
      } // Fecha if(trim($ultimaPlacaCarreta) == trim($placaCarreta2))
      else
      {
    ...
    {
     //Existe a tag, nao precisa adicionar 
     echo "</BR>Ja existe a tag da carreta, nao preciso adiciona-la!</BR>";
    } // fecha if($encontrado_tag == 1)
    else
    {
     //Não exist a tag, tenho que cadastrar no banco!
    ...
    echo "</BR></BR>";
     ...
     {
      echo "</BR></BR>Entrou salvando para Saindo da Planta ou Concluído ";
      echo "</BR></BR>";
     ...
     } // Fecho if($statusProcesso=='Concluído' || $statusProcesso == 'Saindo da Planta')
     else
     {
      if($bloqueia_atuacoes_banco == 0)
      {
     ...
     } // Fecho else if($statusProcesso=='Concluído' || $statusProcesso == 'Saindo da Planta')
     ...
     { 
      ...
      {
       //Existe a tag, nao precisa adicionar 
       echo "</BR>Ja existe a TAG do cavalo, nao preciso adiciona-la</BR>";
      } //Fecho if($encontrado_tag == 1)
      else
      {
       //Não exist a tag, tenho que cadastrar no banco!
       if($bloqueia_atuacoes_banco == 0)
      ...
      }//Fecho else if($encontrado_tag == 1)
     } //Fecha if($epc_cavalo != '-')
     $encontrado_tag = 0;
     if($epc_carreta !='-')
     {
      ...
      {
       //Existe a tag, nao precisa adicionar
       echo "</BR>Ja existe a placa da carreta, nao preciso adiciona-la!</BR>"; 
      }
      else
      {
       //Não exist a tag, tenho que cadastrar no banco!
