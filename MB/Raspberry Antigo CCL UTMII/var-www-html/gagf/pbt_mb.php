...
<img id="voltar" src="./images/btn_voltar.png" onclick="javascript: location.href=`menu_pbt_mb.php?complemento=${criptografia2.value}&check=${criptografia.value}`;"/>
...
$valores_bd_divergencia_mbvl_gagf = ["-","conexao_divergencia_mbvl_gagf_2020.php","conexao_divergencia_mbvl_gagf_2021.php","conexao_divergencia_mbvl_gagf_2022.php","conexao_divergencia_mbvl_gagf_2023.php","conexao_divergencia_mbvl_gagf_2024.php","conexao_divergencia_mbvl_gagf_2025.php"];
 ...
 if($_FILES['file']['name'])
 {
  ...
  if($filename[1] == 'csv' || $filename[1] == 'CSV')
  {
   ...
   while($data = fgetcsv($handle,1000,";"))
   {
    if ($vezes >=2) // Para desconsiderar o campo data na primeira linha e o cabeçalho da segunda OBS Começa a contar com 0
    {
     ...
     if($item21 == "PROCESSO FINALIZADO") //  Se o processao esta finalizado
     {
      ...
      {
       $finalizado_ok--;  // Desconsidera 1 pois nao faz parte de PBT e sim outras funcoes
       // TRATA OS DADOS NEGATIVOS PARA OUTRAS FUNCOES ******************************************************************************************
       // TRATA OS DADOS NEGATIVOS PARA OUTRAS FUNCOES ******************************************************************************************
       // TRATA OS DADOS NEGATIVOS PARA OUTRAS FUNCOES ******************************************************************************************
       // TRATA OS DADOS NEGATIVOS PARA OUTRAS FUNCOES ******************************************************************************************
       if($item5 == "BI-TRUCK-4EIXOS-29T" || $item5 == "CARRETA-5EIXOS-41,5T" || $item5 == "CARRETA TRUCADA-6EIXOS-16M-45T") // Se for os tipos PARA OS NEGATIVOS
       {
        if($item2 == "8000 ") //  Se e 8000 PARA OS NEGATIVOS
        {
         // PODE TRATAR OS DADOS PARA OS NEGATIVOS  PODE TRATAR OS DADOS PARA OS NEGATIVOS PODE TRATAR OS DADOS PARA OS NEGATIVOS PODE TRATAR OS DADOS PARA OS NEGATIVOS   
         // PODE TRATAR OS DADOS PARA OS NEGATIVOS  PODE TRATAR OS DADOS PARA OS NEGATIVOS PODE TRATAR OS DADOS PARA OS NEGATIVOS PODE TRATAR OS DADOS PARA OS NEGATIVOS
         // PODE TRATAR OS DADOS PARA OS NEGATIVOS  PODE TRATAR OS DADOS PARA OS NEGATIVOS PODE TRATAR OS DADOS PARA OS NEGATIVOS PODE TRATAR OS DADOS PARA OS NEGATIVOS
         
         //TRATANDO A DATA  PARA OS NEGATIVOS *************************************************************************************************
         // Trata tudo e caso algum erro nao deixa salvar para não colocar sujeira no banco de dados 
         $data = "";
         $mes = "";
         $ano = "";
         $hora= "";
         if(strlen($item23)==19) // Trata se a data esta certa PARA OS NEGATIVOS
         {
          $data = substr($item23,0,10);
          $mes = substr($item23,3,2);
          $ano = substr($item23,6,4); 
         ...
         //TRATANDO ID PROCESSO GSCS PARA OS NEGATIVOS ***************************************************************************************
         if (strlen($item1)>=7)
         {
          $id_processo_gscs = "";
          $id_processo_gscs = $item1;
         }
         // TRATANDO AS PLACAS PARA OS NEGATIVOS *********************************************************************************************
         // Retira placa e estado
         if (strlen($item4)==10)
         {
          $placa = "";
         ...
         // TRATA TIPO DE VEICULO  PARA OS NEGATIVOS ******************************************************************************************
         $tipo_veiculo = "";
         $tipo_veiculo = $item5;
         // TRATANDO O PESO BRUTO e TARA PARA OS NEGATIVOS ************************************************************************************
         if (strlen($item25)==9 && strlen($item28)==9)
         {
          $tara = "";
          $peso_bruto_mb = "";
          $tara = intval(substr($item25,0,5));
          $peso_bruto_mb = intval(substr($item28,0,5));
         }
         $tipo_operacao = $item22;
         $transportadora = $item11;
         // Verifica se pode salva PARA OS NEGATIVOS
         
         $valor_mes_inteiro = intval($mes); // Busca o mes da data do processo e converte para posicao do array
         $valor_ano_inteiro = intval($ano)-2019; // Busca o mes da data do processo e converte para posicao do array
         $conexao_divergencia = $valores_bd_divergencia_mbvl_gagf[$valor_ano_inteiro];
         $tabela_divergencia = $valores_mes[$valor_mes_inteiro];
         include_once $conexao_divergencia;
         $sql_divergencia = $aux_conexao_divergencia->query("SELECT * FROM $tabela_divergencia WHERE id_processo_gscs='$id_processo_gscs'"); // Busca se ja existe o n do processo gscs no banco
         if(mysqli_num_rows($sql_divergencia)>0)
         {
          // Se entrar aqui e porque ja exsite este processo salvo no banco e nao deixa duplicas PARA OS NEGATIVOS
          $n_erros_inseridos_nok++; 
         } 
         else // PODE SALVAR POIS NÃO EXISTE ESTE PROCESSO NO BANCO PARA O NEGATIVO
         {
          $peso_bruto_vl = 0; // Salva zero pois sera inserido quando inserido na tela pbt de vl
          $diferenca_percentual = 0.00; // Salva 0 o percento pois sera calculado depois em outra tabela
          $diferenca_quilos = 0; // Salva 0 pois sera calculado depois em outra tabela
          $n_erros_inseridos_ok++;
          if($n_corretos==0){$n_corretos++;}// Insere pois mesmo que todos ja existam no banco aceita o relatorio das nominations negativas ( pesagem bruto )   
          $sql_divergencia = $aux_conexao_divergencia->query("INSERT INTO $tabela_divergencia (data,mes,ano,id_processo_gscs,placa,estado,tipo_veiculo,tara,tipo_operacao,peso_bruto_mb,peso_bruto_vl,diferenca_quilos,diferenca_percentual,transportadora) VALUES ('$data','$mes','$ano','$id_processo_gscs','$placa','$estado','$tipo_veiculo','$tara','$tipo_operacao','$peso_bruto_mb','$peso_bruto_vl','$diferenca_quilos','$diferenca_percentual','$transportadora')");
         } // Fecha else de não pode salvar no banco PARA O NEGATIVO
                
        } // Fecha se centro 8000 PARA O NEGATIVO
       } // Fecha if Se PARA O NEGATIVO for os tipos
       // ***************************************************************************************************************************************
       $negativo_nok++;
   ...
   if($n_inseridos_ok>0)  //Se foram salvo dados, monta a tabela
   {
    ...
    $valor_mes_inteiro = intval($mes); // Busca o mes da data do processo e converte para posicao do array
    $valor_ano_inteiro = intval($ano)-2019; // Busca o mes da data do processo e converte para posicao do array
    $conexao_pbt = $valores_bd_pbt_mb[$valor_ano_inteiro];
...
<script>
  ...
  var array_numero = [];
  var array_placa = [];
  var array_capacidade = [];
  var array_bruto = [];
  var array_assertividade = [];
  var array_transportadora = [];
  var valor = 0;
  var data = "00/00/0000";
  var numero = "00000000";
  var placa = "xxx-0000/00";
  var capacidade = "00000";
  var bruto = "00000";
  var assertividade = "000.000";
  var transportadora = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
  ...
  for (i=0;i<1000;i++)
  {
   ...
   array_numero[i]=0;
   array_placa[i]=0;
   array_capacidade[i]=0;
   array_bruto[i]=0;
   array_assertividade[i]=0;
