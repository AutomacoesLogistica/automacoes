...
<img id="voltar" src="./images/btn_voltar.png" onclick="javascript: location.href=`ttp_uob.php?complemento=${criptografia2.value}&check=${criptografia.value}`;"/>
...
echo"<h3><b> Realizando filtro dos dados com inicio em " .$data_inicio ." até " .$data_fim . " relacionados a UOB</b></h3>";


error_reporting(0);
$valores_mes=["-","janeiro","fevereiro","marco","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"];
$valores_bd_ttp_uob = ["-","conexao_ttp_uob_2020.php","conexao_ttp_uob_2021.php","conexao_ttp_uob_2022.php","conexao_ttp_uob_2023.php","conexao_ttp_uob_2024.php","conexao_ttp_uob_2025.php"];
$valores_bd_ttp_uob_resultado = ["-","conexao_ttp_uob_2020_resultado.php","conexao_ttp_uob_2021_resultado.php","conexao_ttp_uob_2022_resultado.php","conexao_ttp_uob_2023_resultado.php","conexao_ttp_uob_2024_resultado.php","conexao_ttp_uob_2025_resultado.php"];
...
$ano = substr($data_inicio,6,4); 
  ...
  $conexao_ttp = $valores_bd_ttp_uob[$valor_ano_inteiro];
  $conexao_ttp_resultado = $valores_bd_ttp_uob_resultado[$valor_ano_inteiro];
  ...
  $sql_ttp = $aux_conexao_ttp->query("SELECT * FROM $tabela_ttp WHERE data_entrada_usina between '$data_inicio' and '$data_fim'");
  if(mysqli_num_rows($sql_ttp)>0)
  {
     ...
     <table border= 1px; >
      <thead >
       <tr>
    	...
    	<th class="th2">Nº TMC</th>
    	<th class="th3">Placa</th>
        <th class="th4">Material/NI</th>
        <th class="th5">Bruto</th>
        <th class="th6">Tara</th>
   ...
   { 
       ...
       $placa_completa =  strval($dados['placa']);
       $tempo_permanencia = $dados['tempo_permanencia'];
       $verifica_hora = intval(intval(substr($tempo_permanencia,0,2)));
       if($verifica_hora>0)
       {
        $minutos_totais_banco = ( intval($verifica_hora)*60)+ intval(substr($tempo_permanencia,3,2));
        $somatorio_minutos_totais = $somatorio_minutos_totais + (( intval($verifica_hora)*60)+ intval(substr($tempo_permanencia,3,2) )    );
       }
       else
       {
         $minutos_totais_banco = intval(substr($tempo_permanencia,3,2));
         $somatorio_minutos_totais = $somatorio_minutos_totais + intval(substr($tempo_permanencia,3,2)); 
       }
  
       if ( $minutos_totais_banco>=60 )     
       { // Abre o if para somente tempos maiores que 60minutos
        $total++;
        if ( $minutos_totais_banco>=60 && $minutos_totais_banco<120)
        {
         $uma_hora++;
         if($dados['turno']=="A")
         {
         ...
         else if ($dados['turno']=="B")
         {
          $letraB++;
         }
         else if ($dados['turno']=="C")
         {
          $letraC++;   
         }
         else
         ...
         else if( $minutos_totais_banco>=120 && $minutos_totais_banco<180)
         {
          $duas_horas++;
          if($dados['turno']=="A")
          {
         ...
         else if( $minutos_totais_banco>=180 && $minutos_totais_banco<240)
         {
          $tres_horas++;
          if($dados['turno']=="A")
          {
         ...
         else if( $minutos_totais_banco>=240 && $minutos_totais_banco<300)
         {
          $quatro_horas++;
          if($dados['turno']=="A")
          {
         ...
         else if( $minutos_totais_banco>=300 && $minutos_totais_banco<360)
         {
          $cinco_horas++;
          if($dados['turno']=="A")
          {
        ...
        ?>
        <tr>
         <td class="th1"><?php print $dados['data_entrada_usina'];?></td>
         <td class="th2"><?php print $dados['numero_documento'];?></td>
         <td class="th3"><?php print strval($placa_completa);?></td>
         <td class="th4"><?php print $dados['material_ni'];?></td>
         <td class="th5"><?php print $dados['hora_pesou_bruto'];?></td>
         <td class="th6"><?php print $dados['hora_pesou_tara'];?></td>
         <td class="th7"><?php print $dados['tempo_permanencia'];?></td>
         <td class="th8"><?php print $minutos_totais_banco;?></td>
        </tr>
        <?php
