...
<head>
<meta charset="utf-8"/>
<title>VA MB - Cheio/Vazio + Pessoas</title>
...
$link_limite = isset($_GET['limite'])?$_GET['limite']:'1000';
$atualizar = isset($_GET['atualizar'])?$_GET['atualizar']:'nao';

if(intval($link_limite)>20)
{
 $atualizar = "nao"; 
}
...
<script>
  var link_primeiro_id = '0';
  var resposta = '0';
  var resultado2  ='';
  function verifica_videos()
  {
   //console.log('entrou'); 
    setInterval(function() 
    { 
     var link_atualizar = '<?php print $atualizar ?>';
     
    
     if(link_atualizar =='sim')
     {
      console.log(link_primeiro_id);
      
       $.ajax({
           url: 'verifica_videos.php',
           type: 'GET',
           dataType: 'html',
           data: {'id': link_primeiro_id},
     ...
     resposta = resultado2;
     if(link_primeiro_id == '0' && resposta != '')
     {
      link_primeiro_id = resposta;
      //console.log('atualizou ' + resposta);
     }
     else
     {
      if(link_primeiro_id != resposta)
      {
       //console.log(resposta + 'Pode Atualizar!'); 
       if(link_primeiro_id == '0')
       {
        //Nao atualiza ainda
       }
       else
      ...
      {
        console.log(resposta + 'Não Pode Atualizar!'); 
      }
...
<script>
function abrir_cancelas_cheio_vazio(nome_cancela)
{
  //alert(nome_cancela);  
...
function cheio_vazio_altera_video(link_video)
{
 if(link_video == '-')
 {
  link_video = "./video_erro/video.m3u8";
   //alert(link_video);
...
<label id='cheio_vazio_info_eventos4'>Erros SVA: 0 </label>
<label id='cheio_vazio_info_eventos5'>Assertividade SVA: 0% </label>
<?php
date_default_timezone_set('America/Sao_Paulo');
...
?>
<label id='cheio_vazio_titulo'> VA Cheio/Vazio MB - Eventos do dia  <?php print $data ?></label>
<script>
     ...
     <table id='tabela_cheio_vazio'>
        ...
        <tbody id='t_cheio_vazio'>
            ...
            if($cameras == "todas") // OK
            {
             ...
             elseif($status == "apenas_erro_sva")
             {
              $pesquisa = "data_leitura='$data'"." AND caminho='-'";
             }
            ...
            elseif($cameras == "ckb_1") // OK
            {
             ...
             elseif($status == "apenas_erro_sva")
             {
              $pesquisa = "data_leitura='$data'"." AND (camera_id='ms0742-cam00' OR camera_id='ms0742-cam04') AND caminho='-'";
             }
            ...
            elseif($cameras == "ckb_2") // OK
            {
             ...
             elseif($status == "apenas_erro_sva")
             {
              $pesquisa = "data_leitura='$data'"." AND (camera_id='ms0742-cam01' OR camera_id='ms0742-cam05') AND caminho='-'";
               /*
              ?><script>alert("<?php print $pesquisa ?>");</script><?php 
              */
             }
            ...
            elseif($cameras == "ckb_3") // OK
            {
             ...
             elseif($status == "apenas_erro_sva")
             {
              $pesquisa = "data_leitura='$data'"." AND (camera_id='ms0742-cam02' OR camera_id='ms0742-cam03' OR camera_id='ms0742-cam06') AND caminho='-'";
               /*
              ?><script>alert("<?php print $pesquisa ?>");</script><?php 
              */
             }
            ...
            include_once 'conexao_sva.php';
            $sql = $dbcon->query("SELECT * FROM cheio_vazio WHERE ".$pesquisa." ORDER BY id DESC LIMIT $link_limite");
            if(mysqli_num_rows($sql)>0)
            {
             while($dados = $sql->fetch_array())
             {
              ...
              $nome = $dados['nome'];
              $gagf = $dados['gagf'];
              if($gagf == ''){$gagf = '00000000';}
              $gscs = $dados['gscs'];
              if($gscs == ''){$gscs = '00000000';}
              $nomination = $dados['nomination'];
              if($nomination == ''){$nomination = '00000000';}
              $material = $dados['material'];
              if($material == ''){$material = 'xxxxxxxxxxxxxxxxx';}
              $liquido = $dados['liquido'];
              if($liquido == ''){$liquido = '00000Kg';}
              $epc = $dados['epc'];
              if($epc == ''){$epc='000000000000000000000000';}
              $justificativa = $dados['justificativa'];
              $justificativa = str_replace(">",",",$justificativa); // Troca a virgula por > por , para exibir normalmente
              if($justificativa == 'Validou apenas GSCS!')
              {
              ...
              <tr>
                <td class="cheio_vazio_th1">
                  <div id='foto_cheio_vazio'>
                       <?php
                       if($justificativa == 'Validado apenas GSCS!')
                       {
                        $erros_sva = intval($erros_sva)+1;
                       ?>
                       <img id='foto_cheio_vazio' src="./images/sva_nao_validou.png";/>
                       <?php
                       }
                       else if($justificativa == 'SVA não validou a saída')
                       {
                        $erros_sva = intval($erros_sva)+1;
                       ?>
                       <img id='foto_cheio_vazio' src="./images/sva_nao_validou.png";/>
                      ...
                      ?>
                      <label><b>Data Evento : </b> <?php print $data_evento . ' - ' . $hora_evento?></label>
                      </br>
                      <label><b>Placa: </b> <?php print $placa ?></label>
                      
                      
                      <label><b> / </b> <?php print $epc ?></label>
                      </br>
                      <?php
                      if($resumo_transportadora == ''){$resumo_transportadora = 'Não identificado!';}
                      ?>
                      <label><b>Transportadora: </b> <?php print $resumo_transportadora ?></label>
                       </br>
                      <label><b>Status: </b><?php print $status_evento?></label>
                      <label><b>&nbsp&nbsp&nbsp&nbspPeso: </b><?php print $liquido ?></label>  
                    </br>
                      <label><b>GSCS: </b><?php print $gscs ?></label>
                      <label><b>&nbsp&nbsp&nbspGAGF: </b><?php print $gagf ?></label>
                      
                      </br>
                      <label><b>Nomination: </b><?php print $nomination ?></label>
                      </br>
                      <label><b>Material: </b><?php print $material ?></label>
                      </br>
                      ...
                      if($justificado == "SIM")
                      {
                       ...
                       ?> 
                       <a href="javascript:cheio_vazio_altera_justificar_cheio_vazio('<?php echo $id ?>');" style="background-color:#228B22;"  id='assistir' name='assistir'>Parecer Video</a>
                       <script>
                       //alert('verde');
                       </script>
                       <?php
                      ...
                      ?>
                      <a href="javascript:cheio_vazio_altera_justificar_cheio_vazio('<?php echo $id ?>');" style="background-color:#DC143C;"  id='assistir' name='assistir'>Parecer Video</a>
                      <script>
                       //alert('vermelho');
                       </script>
                      <?php
                      ...
                      ?>
                      <a href="javascript:abrir_cancelas_cheio_vazio('<?php echo $cam ?>');" style="background-color:#FF0002;"  id='assistir' name='assistir'>Abrir Cancela</a>
                      
                      <!--<a href="javascript:deteccao_falsa_cheio_vazio('<?php echo $id ?>');" id='ignorar' name='ignorar'>Detecção Falsa</a>-->
                      <label id='lb_fim' name='lb_fim'><b>---------------------------------------------------------- </b></label>
                  </div>
             ...
             <script>
                ...
                n_erros_sva = '<?php print $erros_sva ?>';
                n_justificado = '<?php print $evento_justificado ?>';
                n_aderencia = (n_justificado/n_encontrado * 100).toFixed(1) + "%";
                n_assertividade_entrada_sva = (100-(n_erros_sva/n_encontrado * 100).toFixed(1)) + "%";
                n_assertividade_saida_sva = (100-(n_erros_sva/n_encontrado * 100).toFixed(1)) + "%";
                n_assertividade_saida_alt_sva = (100-(n_erros_sva/n_encontrado * 100).toFixed(1)) + "%";
                //alert(n_justificado);
                ...
                document.getElementById('cheio_vazio_info_eventos4').innerHTML = 'Erros SVA : '+ n_erros_sva;
                
                camera = '<?php print $cameras?>';
                console.log(n_erros_sva);
                if(camera == 'ckb_1') // Filtrado somente a entrada
                {
                  document.getElementById('cheio_vazio_info_eventos5').style.display='block';
                  document.getElementById('cheio_vazio_info_eventos5').innerHTML = 'Assert. SVA (Entrada) : '+ n_assertividade_entrada_sva;
                }
                else if(camera == 'ckb_2') // Filtrado somente a saida
                {
                  document.getElementById('cheio_vazio_info_eventos5').style.display='block';
                  document.getElementById('cheio_vazio_info_eventos5').innerHTML = 'Assert. SVA (Saida) : '+ n_assertividade_saida_sva;
                }
                else if(camera == 'ckb_3') // Filtrado somente a saida alternativa
                {
                  document.getElementById('cheio_vazio_info_eventos5').style.display='block';
                  document.getElementById('cheio_vazio_info_eventos5').innerHTML = 'Assert. SVA (Saida Alt.) : '+ n_assertividade_saida_alt_sva;
                }
                else if(cameras='ckb_1,ckb_2') // Filtrado entrada e saida
                {
                  document.getElementById('cheio_vazio_info_eventos5').style.display='block';
                  document.getElementById('cheio_vazio_info_eventos5').innerHTML = 'Assert. SVA (Ent/Sai) : '+ n_assertividade_saida_sva;
                }
                
                else
                {
                  document.getElementById('cheio_vazio_info_eventos5').style.display='none';
                  document.getElementById('cheio_vazio_info_eventos5').innerHTML = 'Assertividade SVA : 0%';
                }
...
<script>
verifica_videos(); //Para verificar novos videos 
cheio_vazio_altera_video('<?php echo $primeira_url ?>');
var player = videojs('vcheio_vazio_player1');
player.play();
     ...
     <table id='tabela_pessoas'>
        ...
        <tbody id='t_pessoas'>
            ...
            elseif($cameras_pessoas == "pessoas_ckb_1") // OK
            {
             ...
             //ms0742-cam07 - Patio CCL
             $pesquisa_pessoas = "data_leitura='$data'"." AND (camera_id='ms0742-cam07')";
            ...
            else// pessoas_ckb_2
            {
             ...
             //ms0742-cam08 - Abertura I e II
             $pesquisa_pessoas = "data_leitura='$data'"." AND (camera_id='ms0742-cam08')";
            }
...
<label id="lb_filtro4_3">Numero videos : </label>
<input type='text' id='lb_filtro4_4' name='lb_filtro4_4' value='<?php print $link_limite ?>'/>
<input type="checkbox" id="ckb_8" name="ckb_8" checked>
<label id="lb_filtro4_5">Atualizar Automatico</label>
<input type="checkbox" id="ckb_9" name="ckb_9" checked>
<label id="lb_filtro4_6">Erros SVA</label>
...
<label id="pessoas_lb_filtro2_1">CAM Pátio CCL</label>

<input type="checkbox" id="pessoas_ckb_2" name="pessoas_ckb_2" checked>
<label id="pessoas_lb_filtro2_2">CAM Abertura I e II</label>
...
if (document.getElementById('ckb_4').checked == true && document.getElementById('ckb_5').checked == true && document.getElementById('ckb_9').checked == true)
{
 val_cheio_vazio = "todos";
}
else if (document.getElementById('ckb_4').checked == true && document.getElementById('ckb_5').checked == false && document.getElementById('ckb_9').checked == false)
{
  val_cheio_vazio = "apenas_cheio";
}

else if (document.getElementById('ckb_4').checked == false && document.getElementById('ckb_5').checked == true && document.getElementById('ckb_9').checked == false)
{
  val_cheio_vazio = "apenas_vazio";
}
else if (document.getElementById('ckb_4').checked == false && document.getElementById('ckb_5').checked == false && document.getElementById('ckb_9').checked == true)
{
  val_cheio_vazio = "apenas_erro_sva";
}
else{
  val_cheio_vazio = "apenas_erro_sva";
}
...
//VERIFICA A CONDIÇÃO DO ATUALIZAR
if (document.getElementById('ckb_8').checked == true)
{
 v_atualizar = "sim";
}
else
...
var link_limite = document.getElementById('lb_filtro4_4');
link_limite = link_limite.value;
//alert(link_limite);

document.getElementById('div_filtrar_cheio_vazio').style.display = 'none';
location.href=`tela_va_cheio_vazio_mb.php?complemento=${criptografia2.value}&check=${criptografia.value}&data=`+link_data+`&complemento=`+link_complemento+`&check=`+ link_check+`&cameras=`+resp_cameras+`&status=`+resp_cheio_vazio+`&parecer=`+resp_parecer+'&limite='+link_limite+'&atualizar='+v_atualizar;
...
function clicou_salvar_filtro_pessoas()
{
 ...
 //CAMERAS
 if (document.getElementById('pessoas_ckb_1').checked == true && document.getElementById('pessoas_ckb_2').checked == true)
 {
  val_cameras_pessoas = "todas";
 }
 else if (document.getElementById('pessoas_ckb_1').checked == true && document.getElementById('pessoas_ckb_2').checked == false)
 {
 ...
 location.href=`tela_va_cheio_vazio_mb.php?complemento=${criptografia2.value}&check=${criptografia.value}&data=`+link_data+`&complemento=`+link_complemento+`&check=`+link_check+`&status=`+resp_cheio_vazio+`&cameras=`+resp_cameras+`&cameras_pessoas=`+resp_cameras_pessoas+`&parecer=`+resp_parecer+`&parecer_pessoas=`+resp_parecer_pessoas;
...
<script>
  ...
  clicou_cheio_vazio();
    ...
    var link_limite =  '<?php print $link_limite?>';
    var v_atualizar = '<?php print $atualizar ?>';

    document.getElementById('lb_filtro4_4').value = link_limite;
    ...
    if(link_status == "todos")
    {
      ...
      document.getElementById('ckb_9').checked = true;
    ...
    else if(link_status == "apenas_cheio")
    {
      ...
      document.getElementById('ckb_9').checked = false;
    }
    else if(link_status == "apenas_vazio")
    {
      document.getElementById('ckb_4').checked = false;
      document.getElementById('ckb_5').checked = true;
      document.getElementById('ckb_9').checked = false;
    }
    else if(link_status == "apenas_erro_sva")
    {
      //Apenas vazio
      document.getElementById('ckb_4').checked = false;
      document.getElementById('ckb_5').checked = false;
    ...
    if(v_atualizar == "sim")
    {
      document.getElementById('ckb_8').checked = true;
    }
    else
    {
      document.getElementById('ckb_8').checked = false;
    }
...
{
 ?> 
  <label id='lb_infooo' name='lb_infooo'>Limitado em <?php print $link_limite ?> videos</label>
  <?php
}
?>
<img id='img_carregando' src="./images/carregando.gif"/>
<script>
