...
<label id="titulo2">Miguel Burnier</label>
</div>
...
//alert(senha);
 $.ajax({
    ...
    success: function(resultado)
    {
     ...
     {
      alert('Atenção! Usuario ainda não cadastrado no sistema!');   
     }
     else if (resultado == 'igual,igual,igual,igual')
     {
      alert("Atenção! O valor inserido para usuário e senha estão iguais e já foi alterado a senha para este usuário, favo acessar o sistema com a senha definida!");   
     }
     else if (resultado == 'negado,negado,negado,negado')
     {
      alert("Atenção! Acesso negado, o valor inserido de usuário ou senha estão incorretos, favor verificar!");   
     }
     ...
     else
     {
      ...
      else
      {
       ...
       location.href=`tela_va_cheio_vazio_mb.php?complemento=`+complemento+`&check=`+ complemento2+ `&data=${data2.value}`+`&cameras=todas`+`&status=todos`+`&parecer=todos&limite=10`;
